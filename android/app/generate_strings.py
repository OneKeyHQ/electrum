...
def main():
    ...
    for lang_region in [
        name for name in os.listdir(locale_dir) if isdir(join(locale_dir, name))
    ]:
        lang, region = lang_region.split("_")
        catalog = read_catalog(
            join(locale_dir, lang_region, "LC_MESSAGES", "electrum.mo")
        )
    ...
    for lang, region_strings in lang_strings.items():
        ...
        for i, (region, strings) in enumerate(region_strings):
            write_xml(lang if i == 0 else "{}-r{}".format(lang, region), strings, ids)
                ...
                if entry.msgid_plural:
                    if is_pot:
                        ...
                        if quantities is None:
                            raise Exception(
                                "Unknown plural formula '{}': add it to QUANTITIES in {}".format(
                                    formula, SCRIPT_NAME
                                )
                            )
                        catalog[msgid] = {
                            quantities[i]: fix_format(s)
...
def region_order(item):
    region, strings = item
    return (
        region == "CN",  # "zh" must always be simplified Chinese: see comment above.
        len(strings),
    )
...
def is_excluded(src_str):
    return re.search(r"^\W*$", src_str) or src_str in [  # Empty or only punctuation.
        "Auto connect"
    ]  # Clashes with "Auto-connect".
...
def make_ids(strings):
    ids_out = {}
    for id_options in [
        dict(lower=True, squash=True),
        dict(lower=True, squash=False),
        dict(lower=False, squash=False),
    ]:
        ids_in = {s: tuple(str_to_id(s, **id_options).split("_")) for s in strings}
        try:
    ...
    raise Exception(
        "Failed to make unique IDs for the following strings:\n"
...
def str_to_id(s, *, lower, squash):
    ...
    if squash:
        ...
        rstrip = ""
    id = (
        re.sub(pattern, "_", s, flags=re.ASCII)  # Remove invalid characters.
        .lstrip(lstrip)
        .rstrip(rstrip)
    )
    if id in KEYWORDS:
...
def write_xml(res_suffix, strings, ids):
    ...
    output = sorted(
        (
            (ids[src_str], tgt) for src_str, tgt in strings.items() if src_str in ids
        ),  # Crowdin strings may not be in our local source.
    ...
    with open(join(abs_dir_name, base_name), "w", encoding="UTF-8") as f:
        ...
        for id, tgt in output:
            if isinstance(tgt, dict):
                ...
                print(
                    '    <string name="{}">{}</string>'.format(id, str_for_xml(tgt)),
                    file=f,
                )
        print("</resources>", file=f)
