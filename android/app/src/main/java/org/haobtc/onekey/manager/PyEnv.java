...
public final class PyEnv {
        ...
        Futures.withTimeout(
            mExecutorService.submit(
                () ->
                    Daemon.commands
                        .callAttr(
                            PyConstant.GET_FEATURE, MyApplication.getInstance().getDeviceWay())
                        .toString()),
            5,
            TimeUnit.SECONDS,
            mExecutorService);
        ...
        new FutureCallback<String>() {
          ...
          public void onSuccess(String result) {
            ...
            HardwareFeatures features =
                dealWithConnectedDevice(context, HardwareFeatures.objectFromData(result));
      ...
      if (PreferencesManager.contains(context, Constant.DEVICES, features.getDeviceId())) {
        old =
            HardwareFeatures.objectFromData(
                (String)
                    PreferencesManager.get(context, Constant.DEVICES, features.getDeviceId(), ""));
  ...
  public static String createWallet(
    ...
    try {
      walletInfo =
          sCommands.callAttr(PyConstant.CREATE_WALLET_BY_XPUB, walletName, m, n, xPubs).toString();
   ...
   */
  @WorkerThread
  public static List<BalanceInfoDTO> recoveryWallet(BaseActivity activity, String xPubs, boolean hd)
      throws Exception {
    ...
    try {
      ...
      try {
        walletsInfo =
            sCommands
                .callAttr(PyConstant.CREATE_WALLET_BY_XPUB, "", 1, 1, xPubs, new Kwarg("hd", hd))
                .toString();
          ...
          .forEach(
              derivedInfoBean -> {
                ...
                try {
                  ...
                  blanceFiat =
                      balanceStr.substring(balanceStr.indexOf("(")).replace(")", "").trim();
                } catch (Exception e) {
                  e.printStackTrace();
                }
                ...
                String wallet =
                    PreferencesManager.get(
                            activity, Constant.WALLETS, derivedInfoBean.getName(), "")
                        .toString();
  ...
  public static boolean hasBackup(Context context) {
    String keyName =
        PreferencesManager.get(context, "Preferences", Constant.CURRENT_SELECTED_WALLET_NAME, "")
            .toString();
    try {
  ...
  public static List<BalanceInfoDTO> restoreLocalHDWallet(
    ...
    try {
      ...
      String walletsInfo =
          sCommands
              .callAttr(
                  PyConstant.CREATE_HD_WALLET, password, mnemonics, new Kwarg(Constant.Purpose, 49))
              .toString();
                  ...
                  .forEach(
                      derivedInfoBean -> {
                        ...
                        try {
                          ...
                          blance = blanceStr.substring(0, blanceStr.indexOf("(")).trim();
                          blanceFiat =
                              blanceStr.substring(blanceStr.indexOf("(")).replace(")", "").trim();
                        } catch (Exception e) {
                          e.printStackTrace();
                        }
                        balanceCoinInfo.setBalance(blance);
  ...
  public static PyResponse<String> getFeeInfo(
    ...
    try {
      ...
      if (!isDefault) {
        ...
        if (!Strings.isNullOrEmpty(receiver)) map.put("to_address", receiver);
        ...
        info =
            sCommands
                .callAttr(PyConstant.GET_DEFAULT_FEE_DETAILS, new Kwarg("coin", type))
                .toString();
      }
      Logger.json(info);
  ...
  public static PyResponse<String> getCustomFeeInfo(String value) {
    ...
    try {
      String info =
          sCommands
              .callAttr(PyConstant.GET_DEFAULT_FEE_DETAILS, new Kwarg("feerate", value))
              .toString();
      Logger.d(info);
      response.setResult(info);
    } catch (Exception e) {
  ...
  public static PyResponse<TemporaryTxInfo> getBtcFeeByFeeRate(
    ...
    try {
      ...
      String result =
          Daemon.commands
              .callAttr(PyConstant.CALCULATE_FEE, coin, new Gson().toJson(arrayList), "", feeRate)
              .toString();
      Logger.json(result);
  ...
  public static PyResponse<String> exchange(String value) {
    ...
    try {
      String result =
          sCommands.callAttr(PyConstant.EXCHANGE_RATE_CONVERSION, "base", value).toString();
  ...
  public static PyResponse<TransactionInfoBean> signTx(String rawTx, String password) {
    ...
    try {
      String result =
          sCommands.callAttr(PyConstant.SIGN_TX, rawTx, new Kwarg("password", password)).toString();
  ...
  public static PyResponse<Boolean> verifySignature(
    ...
    try {
      boolean verified =
          sCommands
              .callAttr(PyConstant.VERIFY_MESSAGE_SIGNATURE, address, message, signature)
              .toBoolean();
  ...
  public static PyResponse<String> exportMnemonics(String password, String name) {
    ...
    try {
      String result =
          sCommands
              .callAttr(PyConstant.EXPORT_MNEMONICS, password, new Kwarg("name", name))
              .toString();
      response.setResult(result);
    } catch (Exception e) {
      Exception exception = HardWareExceptions.exceptionConvert(e);
  ...
  public static CreateWalletBean createWallet(
    ...
    try {
      ...
      String result =
          sCommands.callAttr(PyConstant.CREATE_WALLET, argList.toArray(new Object[0])).toString();
      CreateWalletBean createWalletBean = CreateWalletBean.objectFromData(result);
      EventBus.getDefault()
          .post(new CreateSuccessEvent(createWalletBean.getWalletInfo().get(0).getName()));
  ...
  public static PyResponse<Void> createDerivedWallet(
    ...
    try {
      String result =
          sCommands
              .callAttr("create_derived_wallet", walletName, password, currencyType, purpose)
              .toString();
      CreateWalletBean createWalletBean = CreateWalletBean.objectFromData(result);
      EventBus.getDefault()
          .post(new CreateSuccessEvent(createWalletBean.getWalletInfo().get(0).getName()));
                  ...
                  .forEach(
                      (walletName) -> {
                        ...
                        WalletInfo localWalletInfo =
                            JSON.parseObject(jsonElement.toString(), WalletInfo.class);
