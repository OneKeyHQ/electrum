...
class TxDb:
    ...
    def get_tx_info(self, address):
        try:
            ...
            self.cursor.execute(
                "SELECT * FROM txinfo WHERE address=? ORDER BY time", (address,)
            )
    ...
    def add_tx_info(self, address, psbt_tx, tx_hash, raw_tx="", failed_info=""):
        try:
            self.create_table(self.cursor)
            self.cursor.execute(
                "INSERT OR IGNORE INTO txinfo VALUES(?, ?, ?, ?, ?, ?)",
    ...
    def add_tx_time_info(self, tx_hash):
        try:
            self.create_save_time_table(self.cursor)
            self.cursor.execute(
                "INSERT OR IGNORE INTO txtimeinfo VALUES(?, ?)", (tx_hash, time.time())
            )
            self.conn.commit()
        except Exception as e:
            raise e
    ...
    def get_received_tx_fee_info(self, tx_hash):
        try:
            ...
            self.cursor.execute(
                "SELECT * FROM receviedtxfeeinfo WHERE tx_hash=?", (tx_hash,)
            )
            result = self.cursor.fetchall()
            tx_list = []
            for info in result:
    ...
    def add_received_tx_fee_info(self, tx_hash, fee):
        try:
            self.create_save_fee_table(self.cursor)
            self.cursor.execute(
                "INSERT OR IGNORE INTO receviedtxfeeinfo VALUES(?, ?)", (tx_hash, fee)
            )
            self.conn.commit()
        except Exception as e:
            raise e
