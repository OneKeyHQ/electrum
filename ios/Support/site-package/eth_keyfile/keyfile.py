...
def create_keyfile_json(
    private_key, password, version=3, kdf="pbkdf2", iterations=None
):
    if version == 3:
...
def _create_v3_keyfile_json(private_key, password, kdf, work_factor=None):
    ...
    if kdf == "pbkdf2":
        ...
        derived_key = _scrypt_hash(
            password, salt=salt, buflen=DKLEN, r=SCRYPT_R, p=SCRYPT_P, n=work_factor,
        )
        kdfparams = {
            "dklen": DKLEN,
            "n": work_factor,
...
def _derive_scrypt_key(crypto, password):
    ...
    derived_scrypt_key = _scrypt_hash(
        password, salt=salt, n=n, r=r, p=p, buflen=buflen,
    )
...
def _scrypt_hash(password, salt, n, r, p, buflen):
    derived_key = scrypt(
        password, salt=salt, key_len=buflen, N=n, r=r, p=p, num_keys=1,
    )
    return derived_key
