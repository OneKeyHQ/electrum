...
class PyWalib:
            ...
            for server in PyWalib.market_server:
                for name, url in server.items():
                    if name == "coinbase":
                        ...
                        return [
                            str(Decimal(rate))
                            for (ccy, rate) in json["data"]["rates"].items()
                            if ccy == to_cur
                        ][0]
                    # if name == "binance":
        ...
        try:
            ...
            if from_cur in PyWalib.symbols_price:
                ...
                return (
                    ...
                    else PyWalib.get_currency(from_cur, "BTC")
                )
            else:
    ...
    def get_transaction(
        ...
        if contract is None:  # create ETH transaction dictionary
            tx_dict = Eth_Transaction.build_transaction(
                ...
                nonce=self.web3.eth.getTransactionCount(
                    self.web3.toChecksumAddress(from_address)
                ),
            ...
            data_for_contract = Eth_Transaction.get_tx_erc20_data_field(
                to_address, token_amount
            )

            # check whether there is sufficient ERC20 token balance
            _, erc20_balance = self.get_balance(
                self.web3.toChecksumAddress(from_address), contract
            )
        ...
        balance = self.web3.fromWei(
            self.web3.eth.getBalance(self.web3.toChecksumAddress(from_address)), "ether"
        )
    ...
    def get_balance(wallet_address, contract=None):
        if contract is None:
            eth_balance = PyWalib.get_web3().fromWei(
                PyWalib.get_web3().eth.getBalance(wallet_address), "ether"
            )
    ...
    def get_transaction_history(address, recovery=False):
        ...
        if len(tx_list) == 0 and not recovery:
            PyWalib.cursor.execute(
                "SELECT * FROM txlist WHERE address=? ORDER BY time DESC Limit 10",
                (address,),
            )
    ...
    def get_tx_from_etherscan(address, url):
        ...
        for transaction in transactions:
            ...
            PyWalib.cursor.execute(
                "INSERT OR IGNORE INTO txlist VALUES(?, ?,?,?)",
                (transaction["hash"], address, time, json.dumps(extra_dict)),
            )
    ...
    def get_transaction_history_fun(address, recovery=False):
        ...
        for server_key, url in speed_list.items():
            if -1 != server_key.find("trezor"):
                if recovery:
                    print(
                        f"get_transaction history from trezor to recovery....{address, url}"
                    )
