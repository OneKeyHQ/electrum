...
from aiorpcx.curio import TaskGroup, TaskTimeout, timeout_after
...
DEFAULT_EXCHANGE = (
    "CoinGecko"  # default exchange should ideally provide historical rates
)
...
class BitcoinAverage(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json(
            "apiv2.bitcoinaverage.com", "/indices/global/ticker/short"
        )
        return dict(
            [
                (r.replace("BTC", ""), Decimal(json[r]["last"]))
                for r in json
                if r != "timestamp"
            ]
        )
...
class BitcoinVenezuela(ExchangeBase):
    async def get_rates(self, ccy):
        ...
        rates = [
            (r, json["BTC"][r]) for r in json["BTC"] if json["BTC"][r] is not None
        ]  # Giving NULL for LTC
        return dict(rates)
    ...
    async def request_history(self, ccy):
        json = await self.get_json(
            "api.bitcoinvenezuela.com", "/historical/index.php?coin=BTC"
        )
...
class BitStamp(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        if ccy in CURRENCIES[self.name()]:
            json = await self.get_json(
                "www.bitstamp.net", f"/api/v2/ticker/btc{ccy.lower()}/"
            )
...
class Bylls(ExchangeBase):
    async def get_rates(self, ccy):
        json = await self.get_json(
            "bylls.com", "/api/price?from_currency=BTC&to_currency=CAD"
        )
...
class Coinbase(ExchangeBase):
    async def get_rates(self, ccy):
        json = await self.get_json(
            "api.coinbase.com", "/v2/exchange-rates?currency=BTC"
        )
...
class CoinCap(ExchangeBase):
    ...
    async def request_history(self, ccy):
        ...
        history = await self.get_json(
            "api.coincap.io", "/v2/assets/bitcoin/history?interval=d1&limit=2000"
        )
        return dict(
            [
                (
                    datetime.utcfromtimestamp(h["time"] / 1000).strftime("%Y-%m-%d"),
                    h["priceUsd"],
                )
...
class CoinDesk(ExchangeBase):
    async def get_currencies(self):
        dicts = await self.get_json(
            "api.coindesk.com", "/v1/bpi/supported-currencies.json"
        )
        return [d["currency"] for d in dicts]

    async def get_rates(self, ccy):
        json = await self.get_json(
            "api.coindesk.com", "/v1/bpi/currentprice/%s.json" % ccy
        )
...
class CoinGecko(ExchangeBase):
    async def get_rates(self, ccy):
        ...
        return dict(
            [(ccy.upper(), Decimal(d["value"])) for ccy, d in json["rates"].items()]
        )
...
class Kraken(ExchangeBase):
    async def get_rates(self, ccy):
        ...
        json = await self.get_json(
            "api.kraken.com", "/0/public/Ticker?pair=%s" % ",".join(pairs)
        )
        return dict(
            (k[-3:], Decimal(float(v["c"][0]))) for k, v in json["result"].items()
        )
...
class Winkdex(ExchangeBase):
    ...
    async def request_history(self, ccy):
        json = await self.get_json(
            "winkdex.com", "/api/v0/series?start_time=1342915200"
        )
...
class FxThread(ThreadJob):
    ...
    def format_amount_and_units(self, btc_balance):
        ...
        return (
            ""
            if rate.is_nan()
            else "%s %s" % (self.value_str(btc_balance, rate), self.ccy)
        )
    ...
    def get_fiat_status_text(self, btc_balance, base_unit, decimal_point):
        ...
        return (
            ...
            % (
                base_unit,
                self.value_str(COIN / (10 ** (8 - decimal_point)), rate),
                self.ccy,
            )
        )
    ...
    def fiat_value(self, satoshis, rate):
        return (
            Decimal("NaN")
            if satoshis is None
            else Decimal(satoshis) / COIN * Decimal(rate)
        )
...
assert globals().get(
    DEFAULT_EXCHANGE
), f"default exchange {DEFAULT_EXCHANGE} does not exist"
