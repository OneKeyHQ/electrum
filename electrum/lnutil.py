...
from . import bitcoin, crypto, ecc, segwit_addr, transaction
from .bip32 import BIP32_PRIME, BIP32Node
from .bitcoin import address_to_script, push_script, redeem_script_to_address
...
from .util import UserFacingException, bfh, bh2u, inv_dict, list_enabled_bits
...
def channel_id_from_funding_tx(
    funding_txid: str, funding_index: int
) -> Tuple[bytes, bytes]:
...
hex_to_bytes = (
    lambda v: v if isinstance(v, bytes) else bytes.fromhex(v) if v is not None else None
)
json_to_keypair = (
    lambda v: v
...
class Config(StoredObject):
    ...
    def validate_params(self, *, funding_sat: int) -> None:
        ...
        if self.dust_limit_sat < bitcoin.DUST_LIMIT_DEFAULT_SAT_LEGACY:
            raise Exception(
                f"{conf_name}. dust limit too low: {self.dust_limit_sat} sat"
            )
        ...
        if self.max_accepted_htlcs < 1:
            raise Exception(
                f"{conf_name}. max_accepted_htlcs too low: {self.max_accepted_htlcs}"
            )
        if self.max_accepted_htlcs > 483:
            raise Exception(
                f"{conf_name}. max_accepted_htlcs too high: {self.max_accepted_htlcs}"
            )
...
class LocalConfig(Config):
    ...
    def from_seed(self, **kwargs):
        ...
        kwargs["payment_basepoint"] = (
            OnlyPubkeyKeypair(static_remotekey)
            if static_remotekey
            else keypair_generator(LnKeyFamily.PAYMENT_BASE)
        )
...
class RemoteConfig(Config):
    ...
    current_per_commitment_point = attr.ib(
        default=None, type=bytes, converter=hex_to_bytes
    )
            ...
            if e != this_bucket:
                raise Exception(
                    "hash is not derivable: {} {} {}".format(
                        bh2u(e.secret), bh2u(this_bucket.secret), this_bucket.index
                    )
                )
...
ShachainElement.__str__ = (
    lambda self: "ShachainElement(" + bh2u(self.secret) + "," + str(self.index) + ")"
)
...
def secret_to_pubkey(secret: int) -> bytes:
    ...
    return ecc.ECPrivkey.from_secret_scalar(secret).get_public_key_bytes(
        compressed=True
    )
...
def derive_privkey(secret: int, per_commitment_point: bytes) -> int:
    ...
    basepoint = secret + ecc.string_to_number(
        sha256(per_commitment_point + basepoint_bytes)
    )
...
def derive_blinded_pubkey(basepoint: bytes, per_commitment_point: bytes) -> bytes:
    k1 = ecc.ECPubkey(basepoint) * ecc.string_to_number(
...
def derive_blinded_privkey(
    basepoint_secret: bytes, per_commitment_secret: bytes
) -> bytes:
...
def make_htlc_tx_inputs(
    ...
    txin = PartialTxInput(
        prevout=TxOutpoint(txid=bfh(htlc_output_txid), out_idx=htlc_output_index),
        nsequence=0,
    )
...
def make_htlc_tx(
    *, cltv_expiry: int, inputs: List[PartialTxInput], output: PartialTxOutput
) -> PartialTransaction:
...
def make_offered_htlc(
    ...
    return (
        ...
        + bytes([opcodes.OP_EQUAL, opcodes.OP_NOTIF, opcodes.OP_DROP])
...
def possible_output_idxs_of_htlc_in_ctx(
    *,
    ...
    other_revocation_pubkey = derive_blinded_pubkey(
        other_conf.revocation_basepoint.pubkey, pcp
    )
...
def make_htlc_tx_with_open_channel(
    *,
    ...
    if is_htlc_success:
        cltv_expiry = 0
    htlc_tx = make_htlc_tx(
        cltv_expiry=cltv_expiry, inputs=htlc_tx_inputs, output=htlc_tx_output
    )
...
def make_commitment_outputs(
    *,
    ...
    to_local = PartialTxOutput(
        scriptpubkey=bfh(local_script), value=to_local_amt // 1000
    )
    to_remote_amt = max(0, remote_amount_msat - fees_per_participant[REMOTE])
    to_remote = PartialTxOutput(
        scriptpubkey=bfh(remote_script), value=to_remote_amt // 1000
    )
...
def make_commitment_output_to_local_witness_script(
    ...
    local_script = (
        bytes([opcodes.OP_IF])
        + bfh(push_script(bh2u(revocation_pubkey)))
        + bytes([opcodes.OP_ELSE])
        + bitcoin.add_number_to_script(to_self_delay)
        + bytes([opcodes.OP_CHECKSEQUENCEVERIFY, opcodes.OP_DROP])
        + bfh(push_script(bh2u(delayed_pubkey)))
        + bytes([opcodes.OP_ENDIF, opcodes.OP_CHECKSIG])
    )
    return local_script
...
def sign_and_get_sig_string(tx: PartialTransaction, local_config, remote_config):
    tx.sign(
        {
            bh2u(local_config.multisig_key.pubkey): (
                local_config.multisig_key.privkey,
                True,
            )
        }
    )
...
_ln_feature_direct_dependencies = defaultdict(
    set
)  # type: Dict[LnFeatures, Set[LnFeatures]]
...
class LnFeatures(IntFlag):
    ...
    _ln_feature_contexts[OPTION_DATA_LOSS_PROTECT_OPT] = (
        LNFC.INIT | LnFeatureContexts.NODE_ANN
    )
    _ln_feature_contexts[OPTION_DATA_LOSS_PROTECT_REQ] = (
        LNFC.INIT | LnFeatureContexts.NODE_ANN
    )
    ...
    _ln_feature_contexts[OPTION_SUPPORT_LARGE_CHANNEL_OPT] = (
        LNFC.INIT | LNFC.NODE_ANN | LNFC.CHAN_ANN_ALWAYS_EVEN
    )
    _ln_feature_contexts[OPTION_SUPPORT_LARGE_CHANNEL_REQ] = (
        LNFC.INIT | LNFC.NODE_ANN | LNFC.CHAN_ANN_ALWAYS_EVEN
    )
...
def ln_compare_features(
    our_features: "LnFeatures", their_features: int
) -> "LnFeatures":
...
class LNPeerAddr:
    ...
    def __repr__(self):
        return (
            f"<LNPeerAddr host={self.host} port={self.port} pubkey={self.pubkey.hex()}>"
        )
...
def extract_nodeid(connect_contents: str) -> Tuple[bytes, str]:
    ...
    if rest == "":
        raise ConnStringFormatError(
            _("At least a hostname must be supplied after the at symbol.")
        )
    try:
        ...
        raise ConnStringFormatError(
            _("Invalid node ID, must be 33 bytes and hexadecimal")
        )
    return node_id, rest
...
class ShortChannelID(bytes):
    ...
    @classmethod
    def normalize(
        cls, data: Union[None, str, bytes, "ShortChannelID"]
    ) -> Optional["ShortChannelID"]:
...
class UpdateAddHtlc:
    ...
    @classmethod
    def from_tuple(
        cls, amount_msat, payment_hash, cltv_expiry, htlc_id, timestamp
    ) -> "UpdateAddHtlc":
        return cls(
            amount_msat=amount_msat,
            payment_hash=payment_hash,
