...
from .ecc import (
    ...
    get_r_and_s_from_sig_string,
    sig_string_from_r_and_s,
)
...
from .lnutil import (
    LN_MAX_FUNDING_SAT,
    LOCAL,
    MIN_FINAL_CLTV_EXPIRY_ACCEPTED,
    NBLOCK_OUR_CLTV_EXPIRY_DELTA,
    RECEIVED,
    REMOTE,
    ChannelConstraints,
    FeeUpdate,
...
from .util import SilentTaskGroup, bfh, bh2u, chunks, ignore_exceptions, log_exceptions

if TYPE_CHECKING:
    from .lnrouter import LNPaymentRoute, RouteEdge
    from .lnworker import LNBackups, LNGossip, LNWallet, LNWorker
...
class Peer(Logger):
    ...
    def _store_raw_msg_if_local_update(
        ...
        if not chan:
            raise Exception(
                f"channel {channel_id.hex()} not found for peer {self.pubkey.hex()}"
            )
        chan.hm.store_local_update_raw_msg(
            raw_msg, is_commitment_signed=is_commitment_signed
        )
    ...
    def on_error(self, payload):
        self.logger.info(
            f"remote peer sent error [DO NOT TRUST THIS MESSAGE]: {payload['data'].decode('ascii')}"
        )
        chan_id = payload.get("channel_id")
        if chan_id in self.temp_id_to_id:
            chan_id = self.temp_id_to_id[chan_id]
        self.ordered_message_queues[chan_id].put_nowait(
            (None, {"error": payload["data"]})
        )
    ...
    async def wait_for_message(self, expected_name, channel_id):
        ...
        if payload.get("error"):
            raise Exception(
                "Remote peer reported error [DO NOT TRUST THIS MESSAGE]: "
    ...
    def on_init(self, payload):
        if self._received_init:
            ...
            return
        self.their_features = LnFeatures(
            int.from_bytes(payload["features"], byteorder="big")
        )
        their_globalfeatures = int.from_bytes(
            payload["globalfeatures"], byteorder="big"
        )
        ...
        if not self.their_features.validate_transitive_dependecies():
            raise GracefulDisconnect(
                "remote did not set all dependencies for the features they sent"
            )
    ...
    def on_announcement_signatures(self, chan: Channel, payload):
        if chan.config[LOCAL].was_announced:
            h, local_node_sig, local_bitcoin_sig = self.send_announcement_signatures(
                chan
            )
        else:
        ...
        while True:
            ...
            while True:
                ...
                if self.gossip_queue.empty():
                    break
            self.logger.debug(
                f"process_gossip {len(chan_anns)} {len(node_anns)} {len(chan_upds)}"
            )
            ...
            for chan_upds_chunk in chunks(chan_upds, 1000):
                ...
                if orphaned:
                    ...
                    for chan_upd_payload in orphaned:
                        short_channel_id = ShortChannelID(
                            chan_upd_payload["short_channel_id"]
                        )
                ...
                if categorized_chan_upds.good:
                    self.logger.debug(
                        f"on_channel_update: {len(categorized_chan_upds.good)}/{len(chan_upds_chunk)}"
                    )
    ...
    async def query_gossip(self):
        ...
        if self.lnworker == self.lnworker.network.lngossip:
            ...
            self.logger.info(
                "Received {} channel ids. (complete: {})".format(len(ids), complete)
            )
    ...
    def make_local_config(
        self, funding_sat: int, push_msat: int, initiator: HTLCOwner
    ) -> LocalConfig:
        channel_seed = os.urandom(32)
        initial_msat = (
            funding_sat * 1000 - push_msat if initiator == LOCAL else push_msat
        )
    ...
    def temporarily_reserve_funding_tx_change_address(func):
        ...
        async def wrapper(self: "Peer", *args, **kwargs):
            ...
            change_addresses = [
                txout.address
                for txout in funding_tx.outputs()
            ...
            try:
                ...
                for addr in change_addresses:
                    self.lnworker.wallet.set_reserved_state_of_address(
                        addr, reserved=False
                    )
    ...
    async def channel_establishment_flow(
        ...
        *,
        ...
        if funding_sat > LN_MAX_FUNDING_SAT:
            raise Exception(
                f"MUST set funding_satoshis to less than 2^24 satoshi. {funding_sat} sat > {LN_MAX_FUNDING_SAT}"
            )
        if push_msat > 1000 * funding_sat:
            raise Exception(
                f"MUST set push_msat to equal or less than 1000 * funding_satoshis: {push_msat} msat > {1000 * funding_sat} msat"
            )
        if funding_sat < lnutil.MIN_FUNDING_SAT:
            raise Exception(
                f"funding_sat too low: {funding_sat} < {lnutil.MIN_FUNDING_SAT}"
            )
        ...
        funding_output = PartialTxOutput.from_address_and_value(
            funding_address, funding_sat
        )
        dummy_output = PartialTxOutput.from_address_and_value(
            ln_dummy_address(), funding_sat
        )
        ...
        channel_id, funding_txid_bytes = channel_id_from_funding_tx(
        ...
        chan.storage["funding_inputs"] = [
            txin.prevout.to_json() for txin in funding_tx.inputs()
        ]
        if isinstance(self.transport, LNTransport):
    ...
    def create_channel_storage(
        self, channel_id, outpoint, local_config, remote_config, constraints
    ):
        chan_dict = {
    ...
    async def on_open_channel(self, payload):
        ...
        if (
            remote_config.initial_msat
            < calc_fees_for_commitment_tx(
                num_htlcs=0, feerate=feerate, is_local_initiator=False
            )[REMOTE]
        ):
            raise Exception(
                "the funder's amount for the initial commitment transaction is not sufficient for full fee payment"
        ...
        if (
            local_config.initial_msat <= 1000 * payload["channel_reserve_satoshis"]
        ...
        sig_64, _ = chan.sign_next_commitment()
        self.send_message(
            "funding_signed", channel_id=channel_id, signature=sig_64,
        )
    ...
    async def reestablish_channel(self, chan: Channel):
        ...
        if chan.is_static_remotekey_enabled():
            ...
            latest_secret, latest_point = chan.get_secret_and_point(
                LOCAL, latest_local_ctn
            )
        ...
        their_claim_of_our_last_per_commitment_secret = msg.get(
        ...
        if their_oldest_unrevoked_remote_ctn < 0:
            raise RemoteMisbehaving(
                f"channel reestablish: their_oldest_unrevoked_remote_ctn < 0"
            )
        ...
        for ctn, messages in unacked.items():
            ...
            for raw_upd_msg in messages:
                ...
                n_replayed_msgs += 1
        self.logger.info(
            f"channel_reestablish ({chan.get_id_for_log()}): replayed {n_replayed_msgs} unacked messages"
        )
        ...
        if next_remote_ctn != their_next_local_ctn:
            if their_next_local_ctn == latest_remote_ctn and chan.hm.is_revack_pending(
                REMOTE
            ):
        ...
        def are_datalossprotect_fields_valid() -> bool:
            if (
                their_local_pcp is None
                or their_claim_of_our_last_per_commitment_secret is None
            ):
                return False
            if their_oldest_unrevoked_remote_ctn > 0:
            ...
            if chan.is_static_remotekey_enabled():
                return True
            try:
                __, our_remote_pcp = chan.get_secret_and_point(
                    REMOTE, their_next_local_ctn - 1
                )
        ...
        if not are_datalossprotect_fields_valid():
            raise RemoteMisbehaving(
                "channel_reestablish: data loss protect fields invalid"
            )
        ...
        if they_are_ahead:
            ...
            chan.set_data_loss_protect_remote_pcp(
                their_next_local_ctn - 1, their_local_pcp
            )
            self.lnworker.save_channel(chan)
    ...
    def send_funding_locked(self, chan: Channel):
        ...
        per_commitment_point_second = secret_to_pubkey(
            int.from_bytes(
                get_per_commitment_secret_from_seed(
                    chan.config[LOCAL].per_commitment_secret_seed,
        ...
        self.send_message(
            "funding_locked",
            channel_id=channel_id,
            next_per_commitment_point=per_commitment_point_second,
        )
        if chan.is_funded() and chan.config[LOCAL].funding_locked_received:
            self.mark_open(chan)
    ...
    async def handle_announcements(self, chan: Channel):
        ...
        announcement_signatures_msg = await self.announcement_signatures[
            chan.channel_id
        ].get()
        ...
        if not ecc.verify_signature(
            chan.config[REMOTE].multisig_key.pubkey, remote_bitcoin_sig, h
        ):
    ...
    def mark_open(self, chan: Channel):
        ...
        forwarding_enabled = self.network.config.get(
        ...
        if forwarding_enabled:
            ...
            self.logger.info(
                f"sending channel update for outgoing edge ({chan.get_id_for_log()})"
            )
    ...
    def send_announcement_signatures(self, chan: Channel):
        ...
        bitcoin_signature = ecc.ECPrivkey(chan.config[LOCAL].multisig_key.privkey).sign(
            msg_hash, sig_string_from_r_and_s
        )
        node_signature = ecc.ECPrivkey(self.privkey).sign(
            msg_hash, sig_string_from_r_and_s
        )
        self.send_message(
    ...
    def on_update_fail_htlc(self, chan: Channel, payload):
        ...
        self.logger.info(
            f"on_update_fail_htlc. chan {chan.short_channel_id}. htlc_id {htlc_id}"
        )
    ...
    def pay(
        ...
        *,
        ...
        hops_data, amount_msat, cltv = calc_hops_data_for_payment(
            route, amount_msat, final_cltv, payment_secret=payment_secret
        )
        ...
        if cltv > local_height + lnutil.NBLOCK_CLTV_EXPIRY_TOO_FAR_INTO_FUTURE:
            raise PaymentFailure(
                f"htlc expiry too far into future. (in {cltv-local_height} blocks)"
            )
        htlc = UpdateAddHtlc(
        ...
        self.logger.info(
            f"starting payment. len(route)={len(route)}. route: {route}. "
    ...
    def on_commitment_signed(self, chan: Channel, payload):
        ...
        if not chan.has_pending_changes(LOCAL):
            ...
            raise RemoteMisbehaving(
                "received commitment_signed without pending changes"
            )
        # REMOTE should wait until we have revoked
        if chan.hm.is_revack_pending(LOCAL):
            raise RemoteMisbehaving(
                "received commitment_signed before we revoked previous ctx"
            )
    ...
    def on_update_fulfill_htlc(self, chan: Channel, payload):
        ...
        htlc_id = payload["id"]
        self.logger.info(
            f"on_update_fulfill_htlc. chan {chan.short_channel_id}. htlc_id {htlc_id}"
        )
    ...
    def on_update_fail_malformed_htlc(self, chan: Channel, payload):
        ...
        reason = OnionRoutingFailureMessage(
            code=failure_code, data=payload["sha256_of_onion"]
        )
    ...
    def on_update_add_htlc(self, chan: Channel, payload):
        ...
        htlc_id = payload["id"]
        self.logger.info(
            f"on_update_add_htlc. chan {chan.short_channel_id}. htlc_id {htlc_id}"
        )
        ...
        if cltv_expiry > bitcoin.NLOCKTIME_BLOCKHEIGHT_MAX:
            asyncio.ensure_future(self.lnworker.try_force_closing(chan.channel_id))
            raise RemoteMisbehaving(
                f"received update_add_htlc with cltv_expiry > BLOCKHEIGHT_MAX. value was {cltv_expiry}"
            )
        # add htlc
        htlc = UpdateAddHtlc(
        ...
        *,
        ...
        if not forwarding_enabled:
            ...
            return (
                ...
                None,
                OnionRoutingFailureMessage(
                    code=OnionFailureCode.PERMANENT_CHANNEL_FAILURE, data=b""
                ),
            )
        ...
        try:
            next_chan_scid = processed_onion.hop_data.payload["short_channel_id"][
                "short_channel_id"
            ]
        except:
            return (
        ...
        if next_chan is None:
            self.logger.info(
                f"cannot forward htlc. cannot find next_chan {next_chan_scid}"
            )
            return (
                None,
                None,
                OnionRoutingFailureMessage(
                    code=OnionFailureCode.UNKNOWN_NEXT_PEER, data=b""
                ),
            )
        ...
        if not next_chan.can_send_update_add_htlc():
            self.logger.info(
                f"cannot forward htlc. next_chan {next_chan_scid} cannot send ctx updates. "
            ...
            return (
                ...
                None,
                OnionRoutingFailureMessage(
                    code=OnionFailureCode.TEMPORARY_CHANNEL_FAILURE, data=data
                ),
            )
        try:
            next_cltv_expiry = processed_onion.hop_data.payload["outgoing_cltv_value"][
                "outgoing_cltv_value"
            ]
        except:
            return (
        ...
        if htlc.cltv_expiry - next_cltv_expiry < NBLOCK_OUR_CLTV_EXPIRY_DELTA:
            data = (
                htlc.cltv_expiry.to_bytes(4, byteorder="big")
                + outgoing_chan_upd_len
                + outgoing_chan_upd
            )
            return (
                None,
                None,
                OnionRoutingFailureMessage(
                    code=OnionFailureCode.INCORRECT_CLTV_EXPIRY, data=data
                ),
            )
        if (
            htlc.cltv_expiry - lnutil.MIN_FINAL_CLTV_EXPIRY_ACCEPTED <= local_height
            ...
            return (
                ...
                None,
                OnionRoutingFailureMessage(
                    code=OnionFailureCode.EXPIRY_TOO_SOON, data=data
                ),
            )
        if (
            max(htlc.cltv_expiry, next_cltv_expiry)
            > local_height + lnutil.NBLOCK_CLTV_EXPIRY_TOO_FAR_INTO_FUTURE
        ):
            return (
                None,
                None,
                OnionRoutingFailureMessage(
                    code=OnionFailureCode.EXPIRY_TOO_FAR, data=b""
                ),
            )
        try:
        ...
        if htlc.amount_msat - next_amount_msat_htlc < forwarding_fees:
            data = (
                next_amount_msat_htlc.to_bytes(8, byteorder="big")
                + outgoing_chan_upd_len
                + outgoing_chan_upd
            )
            return (
                ...
                None,
                OnionRoutingFailureMessage(
                    code=OnionFailureCode.FEE_INSUFFICIENT, data=data
                ),
            )
    ...
    def maybe_fulfill_htlc(
        ...
        *,
        ...
        try:
            payment_secret_from_onion = processed_onion.hop_data.payload[
                "payment_data"
            ]["payment_secret"]
        except:
            pass  # skip
            ...
            if (
                ...
                return None, reason
        expected_received_msat = (
            int(info.amount * 1000) if info.amount is not None else None
        )
        if expected_received_msat is not None and not (
            expected_received_msat <= htlc.amount_msat <= 2 * expected_received_msat
        ):
            reason = OnionRoutingFailureMessage(
                code=OnionFailureCode.INCORRECT_OR_UNKNOWN_PAYMENT_DETAILS, data=b""
            )
        ...
        if local_height + MIN_FINAL_CLTV_EXPIRY_ACCEPTED > htlc.cltv_expiry:
            reason = OnionRoutingFailureMessage(
                code=OnionFailureCode.FINAL_EXPIRY_TOO_SOON, data=b""
            )
            return None, reason
        try:
            cltv_from_onion = processed_onion.hop_data.payload["outgoing_cltv_value"][
                "outgoing_cltv_value"
            ]
        except:
            reason = OnionRoutingFailureMessage(
        ...
        if cltv_from_onion != htlc.cltv_expiry:
            ...
            return None, reason
        try:
            amount_from_onion = processed_onion.hop_data.payload["amt_to_forward"][
                "amt_to_forward"
            ]
        except:
            reason = OnionRoutingFailureMessage(
            ...
            return None, reason
        try:
            amount_from_onion = processed_onion.hop_data.payload["payment_data"][
                "total_msat"
            ]
        except:
    ...
    def fulfill_htlc(self, chan: Channel, htlc_id: int, preimage: bytes):
        self.logger.info(
            f"_fulfill_htlc. chan {chan.short_channel_id}. htlc_id {htlc_id}"
        )
        assert (
            chan.can_send_ctx_updates()
        ), f"cannot send updates: {chan.short_channel_id}"
    ...
    def fail_malformed_htlc(
        self, *, chan: Channel, htlc_id: int, reason: OnionRoutingFailureMessage
    ):
        self.logger.info(
            f"fail_malformed_htlc. chan {chan.short_channel_id}. htlc_id {htlc_id}."
        )
        assert (
            chan.can_send_ctx_updates()
        ), f"cannot send updates: {chan.short_channel_id}"
        chan.fail_htlc(htlc_id)
        if not (
            reason.code & OnionFailureCodeMetaFlag.BADONION and len(reason.data) == 32
        ):
            raise Exception(
    ...
    def on_revoke_and_ack(self, chan: Channel, payload):
        ...
        rev = RevokeAndAck(
            payload["per_commitment_secret"], payload["next_per_commitment_point"]
        )
    ...
    async def maybe_update_fee(self, chan: Channel):
        ...
        self.send_message(
            "update_fee", channel_id=chan.channel_id, feerate_per_kw=feerate_per_kw
        )
        self.maybe_send_commitment(chan)
    ...
    async def on_shutdown(self, chan: Channel, payload):
        ...
        if not (
            ...
            or match_script_against_template(
                their_scriptpubkey, transaction.SCRIPTPUBKEY_TEMPLATE_P2SH
            )
            or match_script_against_template(
                their_scriptpubkey, transaction.SCRIPTPUBKEY_TEMPLATE_P2PKH
            )
        ):
            raise Exception(
        ...
        if chan_id in self.shutdown_received:
            ...
            self.logger.info(
                f"({chan.get_id_for_log()}) Channel closed by remote peer {txid}"
            )
    ...
    def can_send_shutdown(self, chan):
        ...
        if (
            ...
            and chan.channel_id in self.funding_created_sent
        ):
            return True
        if (
    ...
    async def _shutdown(self, chan: Channel, payload, is_local):
        ...
        while len(chan.hm.htlcs(LOCAL)) + len(chan.hm.htlcs(REMOTE)) > 0:
            self.logger.info(
                f"(chan: {chan.short_channel_id}) waiting for htlcs to settle..."
            )
            await asyncio.sleep(1)
        ...
        our_sig, closing_tx = chan.make_closing_tx(
            our_scriptpubkey, their_scriptpubkey, fee_sat=0
        )
        ...
        while True:
            ...
            if their_fee > max_fee:
                raise Exception(
                    f"the proposed fee exceeds the base fee of the latest commitment transaction {is_local, their_fee, max_fee}"
                )
    ...
    async def htlc_switch(self):
        ...
        while True:
            ...
            for chan_id, chan in self.channels.items():
                ...
                for (
                    htlc_id,
                    (local_ctn, remote_ctn, onion_packet_hex, forwarding_info),
                    ...
                    try:
                        ...
                        error_reason = OnionRoutingFailureMessage(
                            code=OnionFailureCode.INVALID_ONION_VERSION,
                            data=sha256(onion_packet_bytes),
                        )
                    except InvalidOnionPubkey:
                        error_reason = OnionRoutingFailureMessage(
                            code=OnionFailureCode.INVALID_ONION_KEY,
                            data=sha256(onion_packet_bytes),
                        )
                    except InvalidOnionMac:
                        error_reason = OnionRoutingFailureMessage(
                            code=OnionFailureCode.INVALID_ONION_HMAC,
                            data=sha256(onion_packet_bytes),
                        )
                    except Exception as e:
                        ...
                        if self.network.config.get(
                            "test_fail_htlcs_with_temp_node_failure"
                        ):
                            error_reason = OnionRoutingFailureMessage(
                                code=OnionFailureCode.TEMPORARY_NODE_FAILURE, data=b""
                            )
                    ...
                    if not error_reason:
                        if processed_onion.are_we_final:
                            ...
                            next_chan = self.lnworker.get_channel_by_short_id(
                            ...
                            if next_chan:
                                (
                                    error_bytes,
                                    error_reason,
                                ) = next_chan.pop_fail_htlc_reason(htlc_id)
                        if preimage:
                    ...
                    if error_reason or error_bytes:
                        ...
                        if error_bytes:
                            self.fail_htlc(
                                chan=chan, htlc_id=htlc.htlc_id, error_bytes=error_bytes
                            )
                        else:
