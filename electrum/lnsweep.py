...
from . import ecc
from .bitcoin import dust_threshold, redeem_script_to_address
from .lnutil import (
    LOCAL,
    RECEIVED,
    REMOTE,
...
def create_sweeptxs_for_our_ctx(
    *, chan: "AbstractChannel", ctx: Transaction, sweep_address: str
    ...
    our_pcp = ecc.ECPrivkey(our_per_commitment_secret).get_public_key_bytes(
        compressed=True
    )
    our_delayed_bp_privkey = ecc.ECPrivkey(our_conf.delayed_basepoint.privkey)
    our_localdelayed_privkey = derive_privkey(
        our_delayed_bp_privkey.secret_scalar, our_pcp
    )
    our_localdelayed_privkey = ecc.ECPrivkey.from_secret_scalar(
        our_localdelayed_privkey
    )
    their_revocation_pubkey = derive_blinded_pubkey(
        their_conf.revocation_basepoint.pubkey, our_pcp
    )
    ...
    our_htlc_privkey = derive_privkey(
        ...
        per_commitment_point=our_pcp,
    ).to_bytes(32, "big")
    our_localdelayed_pubkey = our_localdelayed_privkey.get_public_key_bytes(
        compressed=True
    )
    to_local_witness_script = bh2u(
        make_commitment_output_to_local_witness_script(
    ...
    their_payment_pubkey = (
        bpk
        if chan.is_static_remotekey_enabled()
        else derive_pubkey(their_conf.payment_basepoint.pubkey, our_pcp)
    )
    ...
    if not ctx.get_output_idxs_from_address(
        to_local_address
...
def analyze_ctx(chan: "Channel", ctx: Transaction):
    ...
    if ctn == oldest_unrevoked_remote_ctn:
        ...
        try:
            ...
            return
        their_pcp = ecc.ECPrivkey(per_commitment_secret).get_public_key_bytes(
            compressed=True
        )
        is_revocation = True
...
def create_sweeptxs_for_their_ctx(
    *, chan: "Channel", ctx: Transaction, sweep_address: str
    ...
    our_revocation_pubkey = derive_blinded_pubkey(
        our_conf.revocation_basepoint.pubkey, their_pcp
    )
    ...
    our_payment_pubkey = (
        bpk if chan.is_static_remotekey_enabled() else derive_pubkey(bpk, their_pcp)
    )
    ...
    if not chan.is_static_remotekey_enabled():
        ...
        our_payment_privkey = derive_privkey(
            our_payment_bp_privkey.secret_scalar, their_pcp
        )
        ...
        if output_idxs:
            ...
            txs[prevout] = SweepInfo(
                name="their_ctx_to_remote", csv_delay=0, cltv_expiry=0, gen_tx=sweep_tx
            )
    # HTLCs
    def create_sweeptx_for_htlc(
        htlc: "UpdateAddHtlc", is_received_htlc: bool, ctx_output_idx: int
    ) -> None:
        ...
        sweep_tx = lambda: create_sweeptx_their_ctx_htlc(
            ...
            privkey=our_revocation_privkey
            if is_revocation
            else our_htlc_privkey.get_secret_bytes(),
...
def create_htlctx_that_spends_from_our_ctx(
    ...
    remote_htlc_sig = chan.get_remote_htlc_sig_for_htlc(
        htlc_relative_idx=htlc_relative_idx
    )
    local_htlc_sig = bfh(htlc_tx.sign_txin(0, local_htlc_privkey))
    txin = htlc_tx.inputs()[0]
    witness_program = bfh(Transaction.get_preimage_script(txin))
    txin.witness = make_htlc_tx_witness(
        remote_htlc_sig, local_htlc_sig, preimage, witness_program
    )
...
def create_sweeptx_their_ctx_htlc(
    ...
    preimage = (
        preimage or b""
    )  # preimage is required iff (not is_revocation and htlc is offered)
    ...
    tx = PartialTransaction.from_io(
        sweep_inputs, sweep_outputs, version=2, locktime=cltv_expiry
    )
