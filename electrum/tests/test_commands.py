...
class TestCommands(ElectrumTestCase):
    ...
    def test_export_private_key_imported(self, mock_save_db):
        ...
        with self.assertRaises(Exception):
            cmds._run(
                "getprivatekeys", ("asdasd",), wallet=wallet
            )  # invalid addr, though might raise "not in wallet"
        with self.assertRaises(Exception):
            cmds._run(
                "getprivatekeys",
                ("bc1qgfam82qk7uwh5j2xxmcd8cmklpe0zackyj6r23",),
                wallet=wallet,
            )  # not in wallet
        self.assertEqual(
        ...
        with self.assertRaises(Exception):
            cmds._run(
                "getprivatekeys",
                (["bc1q2ccr34wzep58d4239tl3x3734ttle92a8srmuw", "asd"],),
                wallet=wallet,
            )
        self.assertEqual(
            [
    ...
    def test_export_private_key_deterministic(self, mock_save_db):
        ...
        with self.assertRaises(Exception):
            cmds._run(
                "getprivatekeys",
                (["bc1q3g5tmkmlvxryhh843v4dz026avatc0zzr6h3af", "asd"],),
                wallet=wallet,
            )
        self.assertEqual(
            [
