...
from electrum.util import MyEncoder, bfh, bh2u
...
local_privkey = bytes.fromhex(
    "bb13b121cdc357cd2e608b0aea294afca36e2b34cf958e2e6451a2f27469449101"
)
localpubkey = bytes.fromhex(
    "030d417a46946384f88d5f3337267c5e579765875dc4daca813e21734b140639e7"
)
remotepubkey = bytes.fromhex(
...
class TestLNUtil(ElectrumTestCase):
            ...
            for insert in test["inserts"]:
                ...
                try:
                    ...
                    if not insert["successful"]:
                        raise Exception(
                            "Failed ({}): error wasn't received".format(test["name"])
                        )
    ...
    def test_commitment_tx_with_all_five_HTLCs_untrimmed_minimum_feerate(self):
        ...
        htlc[2] = make_offered_htlc(
            ...
            remote_htlcpubkey,
            local_htlcpubkey,
            bitcoin.sha256(htlc_payment_preimage[2]),
        )
        ...
        htlc[3] = make_offered_htlc(
            ...
            remote_htlcpubkey,
            local_htlcpubkey,
            bitcoin.sha256(htlc_payment_preimage[3]),
        )
        ...
        htlc[0] = make_received_htlc(
            ...
            remote_htlcpubkey,
            local_htlcpubkey,
            bitcoin.sha256(htlc_payment_preimage[0]),
            htlc_cltv_timeout[0],
        )
        ...
        htlc[1] = make_received_htlc(
            ...
            remote_htlcpubkey,
            local_htlcpubkey,
            bitcoin.sha256(htlc_payment_preimage[1]),
            htlc_cltv_timeout[1],
        )
        ...
        htlc[4] = make_received_htlc(
            ...
            remote_htlcpubkey,
            local_htlcpubkey,
            bitcoin.sha256(htlc_payment_preimage[4]),
            htlc_cltv_timeout[4],
        )
    ...
    def htlc_tx(
        ...
        our_htlc_tx_witness = make_htlc_tx_witness(
            ...
            if success
            else b"",  # will put 00 on witness if timeout
            witness_script=htlc,
        )
    ...
    def test_extract_commitment_number_from_tx(self):
        ...
        self.assertEqual(
            commitment_number,
            extract_ctn_from_tx(
                tx, 0, local_payment_basepoint, remote_payment_basepoint
            ),
        )
    ...
    def test_key_derivation(self):
        ...
        revocationpubkey = derive_blinded_pubkey(
    ...
    def sign_and_insert_remote_sig(
        ...
        tx.add_signature_to_txin(
            txin_idx=0, signing_pubkey=remote_pubkey.hex(), sig=remote_signature + "01"
        )
    ...
    def test_extract_nodeid(self):
        ...
        self.assertEqual(
            extract_nodeid("00" * 33 + "@localhost"), (b"\x00" * 33, "localhost")
        )
    ...
    def test_ln_features_for_invoice(self):
        ...
        self.assertEqual(
            LnFeatures.BASIC_MPP_OPT | LnFeatures.PAYMENT_SECRET_REQ,
    ...
    def test_lnworker_decode_channel_update_msg(self):
        ...
        # good messages
        self.assertNotEqual(
            None, LNWallet._decode_channel_update_msg(msg_without_prefix)
        )
        self.assertNotEqual(
            None,
            LNWallet._decode_channel_update_msg(
                bytes.fromhex("0102") + msg_without_prefix
            ),
        )
        # bad messages
        self.assertEqual(
            None, LNWallet._decode_channel_update_msg(bytes.fromhex("0102030405"))
        )
        self.assertEqual(
            None,
            LNWallet._decode_channel_update_msg(
                bytes.fromhex("ffff") + msg_without_prefix
            ),
        )
        self.assertEqual(
            None,
            LNWallet._decode_channel_update_msg(
                bytes.fromhex("0101") + msg_without_prefix
            ),
        )
