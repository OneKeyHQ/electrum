...
from electrum import bip32, constants, crypto, ecc, ecc_fast
...
from electrum.util import InvalidPassword, bfh, bh2u, randrange

from . import FAST_TESTS, ElectrumTestCase, TestCaseForTestnet

try:
    import ecdsa
except ImportError:
    sys.exit(
        "Error: python-ecdsa does not seem to be installed. Try 'sudo python3 -m pip install ecdsa'"
    )
...
class Test_bitcoin(ElectrumTestCase):
    ...
    def test_chacha20_poly1305_encrypt__without_associated_data(self):
        ...
        self.assertEqual(
            ...
            crypto.chacha20_poly1305_encrypt(
                key=key, nonce=nonce, data=data, associated_data=b""
            ),
        )
    ...
    def test_chacha20_encrypt(self):
        ...
        self.assertEqual(
            bytes.fromhex(
                "f62fbd74d197323c7c3d5658476a884d38ee6f4b5500add1e8dc80dcd9c15dff"
            ),
...
class Test_xprv_xpub(ElectrumTestCase):
    ...
    def _do_test_bip32(self, seed: str, sequence):
        ...
        for n in sequence.split("/"):
            if n[-1] != "'":
                xpub2 = (
                    BIP32Node.from_xkey(xpub).subkey_at_public_derivation(n).to_xpub()
                )
    ...
    def test_convert_bip32_intpath_to_strpath(self):
        ...
        self.assertEqual(
            "m/44'/5241'/221",
            convert_bip32_intpath_to_strpath([2147483692, 2147488889, 221]),
        )
