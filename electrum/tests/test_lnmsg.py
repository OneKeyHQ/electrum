...
class TestLNMsg(TestCaseForTestnet):
    ...
    def test_read_bigsize_int(self):
        ...
        self.assertEqual(
            4294967296, read_bigsize_int(io.BytesIO(bfh("ff0000000100000000")))
        )
        self.assertEqual(
    ...
    def test_read_tlv_stream_tests1(self):
            ...
            with self.subTest(tlv_stream_name=tlv_stream_name):
                with self.assertRaises(UnexpectedEndOfStream):
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("fd")), tlv_stream_name=tlv_stream_name
                    )
                with self.assertRaises(UnexpectedEndOfStream):
                    lnser.read_tlv_stream(
                ...
                with self.assertRaises(UnknownMandatoryTLVRecordType):
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("fe0100000200")),
                        tlv_stream_name=tlv_stream_name,
                    )
                with self.assertRaises(UnknownMandatoryTLVRecordType):
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("ff010000000000000200")),
                        tlv_stream_name=tlv_stream_name,
                    )
        ...
        with self.assertRaises(FieldEncodingNotMinimal):
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("0103000100")), tlv_stream_name="n1"
            )
        with self.assertRaises(FieldEncodingNotMinimal):
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("010400010000")), tlv_stream_name="n1"
            )
        with self.assertRaises(FieldEncodingNotMinimal):
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("01050001000000")), tlv_stream_name="n1"
            )
        with self.assertRaises(FieldEncodingNotMinimal):
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("0106000100000000")), tlv_stream_name="n1"
            )
        with self.assertRaises(FieldEncodingNotMinimal):
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("010700010000000000")), tlv_stream_name="n1"
            )
        with self.assertRaises(FieldEncodingNotMinimal):
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("01080001000000000000")), tlv_stream_name="n1"
            )
        with self.assertRaises(UnexpectedEndOfStream):
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("020701010101010101")), tlv_stream_name="n1"
            )
        with self.assertRaises(MsgTrailingGarbage):
            lnser.read_tlv_stream(
        ...
        with self.assertRaises(UnexpectedEndOfStream):
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("fd00fe0101")), tlv_stream_name="n1"
            )
        with self.assertRaises(MsgTrailingGarbage):
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("fd00fe03010101")), tlv_stream_name="n1"
            )
        with self.assertRaises(UnknownMandatoryTLVRecordType):
    ...
    def test_read_tlv_stream_tests2(self):
        ...
        for tlv_stream_name in ("n1", "n2"):
            with self.subTest(tlv_stream_name=tlv_stream_name):
                self.assertEqual(
                    {},
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("")), tlv_stream_name=tlv_stream_name
                    ),
                )
                self.assertEqual(
                    {},
                ...
                self.assertEqual(
                    {},
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("ff020000000000000100")),
                        tlv_stream_name=tlv_stream_name,
                    ),
                )
        ...
        self.assertEqual(
            ...
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("0103010000")), tlv_stream_name="n1"
            ),
        )
        self.assertEqual(
            {"tlv1": {"amount_msat": 16777216}},
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("010401000000")), tlv_stream_name="n1"
            ),
        )
        self.assertEqual(
            {"tlv1": {"amount_msat": 4294967296}},
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("01050100000000")), tlv_stream_name="n1"
            ),
        )
        self.assertEqual(
            {"tlv1": {"amount_msat": 1099511627776}},
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("0106010000000000")), tlv_stream_name="n1"
            ),
        )
        self.assertEqual(
            {"tlv1": {"amount_msat": 281474976710656}},
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("010701000000000000")), tlv_stream_name="n1"
            ),
        )
        self.assertEqual(
            {"tlv1": {"amount_msat": 72057594037927936}},
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("01080100000000000000")), tlv_stream_name="n1"
            ),
        )
        self.assertEqual(
            {"tlv2": {"scid": ShortChannelID.from_components(0, 0, 550)}},
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("02080000000000000226")), tlv_stream_name="n1"
            ),
        )
        self.assertEqual(
            {
        ...
        self.assertEqual(
            {"tlv4": {"cltv_delta": 550}},
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("fd00fe020226")), tlv_stream_name="n1"
            ),
        )
    ...
    def test_read_tlv_stream_tests3(self):
        ...
        with self.assertRaises(MsgInvalidFieldOrder):
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("0208000000000000023102080000000000000451")),
                tlv_stream_name="n1",
            )
        with self.assertRaises(MsgInvalidFieldOrder):
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("1f000f012a")), tlv_stream_name="n1"
            )
        with self.assertRaises(MsgInvalidFieldOrder):
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("1f001f012a")), tlv_stream_name="n1"
            )
        with self.assertRaises(MsgInvalidFieldOrder):
            lnser.read_tlv_stream(
    ...
    def test_encode_decode_msg__missing_optional_field_will_not_appear_in_decoded_dict(
        ...
        self.assertEqual(
            bfh(
                ""
            ),
            encode_msg(
                "channel_update",
                short_channel_id=ShortChannelID.from_components(54321, 111, 2),
    ...
    def test_encode_decode_msg__ints_can_be_passed_as_bytes(self):
        self.assertEqual(
            ...
            encode_msg(
                ...
                channel_flags=b"\x00",
                message_flags=b"\x01",
                cltv_expiry_delta=int.to_bytes(
                    144, length=2, byteorder="big", signed=False
                ),
                htlc_minimum_msat=int.to_bytes(
                    200, length=8, byteorder="big", signed=False
                ),
                htlc_maximum_msat=int.to_bytes(
                    1_000_000_000, length=8, byteorder="big", signed=False
                ),
                fee_base_msat=int.to_bytes(
                    500, length=4, byteorder="big", signed=False
                ),
                ...
                timestamp=int.to_bytes(
                    1584320643, length=4, byteorder="big", signed=False
                ),
            ),
        )
        self.assertEqual(
        ...
        with self.assertRaises(UnexpectedFieldSizeForEncoder):
            encode_msg(
                ...
                htlc_minimum_msat=int.to_bytes(
                    200, length=4, byteorder="big", signed=False
                ),
                htlc_maximum_msat=int.to_bytes(
                    1_000_000_000, length=8, byteorder="big", signed=False
                ),
