...
class TestBolt11(ElectrumTestCase):
    ...
    def compare(a, b):
        ...
        if len([t[1] for t in a.tags if t[0] == "h"]) == 1:
            h1 = sha256(
                [t[1] for t in a.tags if t[0] == "h"][0].encode("utf-8")
            ).digest()
        ...
        assert b.pubkey.serialize() == PUBKEY, (
            hexlify(b.pubkey.serialize()),
            hexlify(PUBKEY),
        )
    ...
    def test_roundtrip(self):
        ...
        tests = [
            ...
            LnAddr(
                paymenthash=RHASH, amount=Decimal("1"), tags=[("h", longdescription)]
            ),
            LnAddr(
                paymenthash=RHASH,
                currency="tb",
            ...
            LnAddr(
                paymenthash=RHASH, amount=24, tags=[("h", longdescription), ("9", 514)]
            ),
            LnAddr(
                paymenthash=RHASH,
                amount=24,
            ...
            LnAddr(
                paymenthash=RHASH,
                amount=24,
                tags=[("h", longdescription), ("9", 10 + (1 << 7) + (1 << 11))],
            ),
            LnAddr(
                paymenthash=RHASH,
                amount=24,
    ...
    def test_n_decoding(self):
        ...
        hrp, data = bech32_decode(
            lnencode(
                LnAddr(paymenthash=RHASH, amount=24, tags=[("d", ""), ("n", PUBKEY)]),
                PRIVKEY,
            ),
            True,
        )
