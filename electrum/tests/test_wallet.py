...
from electrum.wallet_db import FINAL_SEED_VERSION, WalletDB
...
class FakeWallet:
    ...
    def get_tx_height(self, txid):
        ...
        return TxMinedInfo(
            height=10, conf=10, timestamp=int(time.time()), header_hash="def"
        )
...
class TestFiat(ElectrumTestCase):
    ...
    def test_save_fiat_and_reset(self):
        ...
        self.assertEqual(
            "1,000.01", self.fx.ccy_amount_str(Decimal(saved), commas=True)
        )
        self.assertEqual(
            True,
            Abstract_Wallet.set_fiat_value(
                self.wallet, txid, ccy, "", self.fx, self.value_sat
            ),
        )
...
class TestCreateRestoreWallet(WalletTestCase):
    ...
    def test_restore_wallet_from_text_xpub(self):
        ...
        d = restore_wallet_from_text(
            text, path=self.wallet_path, gap_limit=1, config=self.config
        )
        wallet = d["wallet"]  # type: Standard_Wallet
    ...
    def test_restore_wallet_from_text_xkey_that_is_also_a_valid_electrum_seed_by_chance(
        ...
        self.assertEqual(
            "3Pa4hfP3LFWqa2nfphYaF7PZfdJYNusAnp", wallet.get_receiving_addresses()[0]
        )
