...
def load_library():
    ...
    try:
        ...
        _logger.warning(
            "libsecp256k1 library was found and loaded but there was an error when using it"
        )
        return None
...
def _prepare_monkey_patching_of_python_ecdsa_internals_with_libsecp256k1():
    ...
    def add(
        ...
        r = _libsecp256k1.secp256k1_ec_pubkey_combine(
    ...
    def mul(self: ecdsa.ellipticcurve.Point, other: int) -> ecdsa.ellipticcurve.Point:
        ...
        r = _libsecp256k1.secp256k1_ec_pubkey_tweak_mul(
    ...
    def sign(
        self: ecdsa.ecdsa.Private_key, hash: int, random_k: int
        ...
        def sign_with_extra_entropy(extra_entropy):
            ret = _libsecp256k1.secp256k1_ecdsa_sign(
                ...
                sig,
                sig_hash_bytes,
                secret_exponent.to_bytes(32, byteorder="big"),
                nonce_function,
                extra_entropy,
            )
            if not ret:
    ...
    def verify(
        ...
        sig = create_string_buffer(64)
        input64 = signature.r.to_bytes(32, byteorder="big") + signature.s.to_bytes(
            32, byteorder="big"
        )
        r = _libsecp256k1.secp256k1_ecdsa_signature_parse_compact(
            _libsecp256k1.ctx, sig, input64
        )
        if not r:
            return False
        r = _libsecp256k1.secp256k1_ecdsa_signature_normalize(
            _libsecp256k1.ctx, sig, sig
        )

        public_pair_bytes = (
            b"\4"
            + self.point.x().to_bytes(32, byteorder="big")
