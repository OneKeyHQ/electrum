...
def _assert_can_read_at_least_n_bytes(fd: io.BytesIO, n: int) -> None:
    ...
    if nremaining < n:
        raise UnexpectedEndOfStream(
            f"wants to read {n} bytes but only {nremaining} bytes left"
        )
...
def _read_field(
    *, fd: io.BytesIO, field_type: str, count: Union[int, str]
) -> Union[bytes, int]:
    if not fd:
        raise Exception()
    ...
    *, fd: io.BytesIO, field_type: str, count: Union[int, str], value: Union[bytes, int]
    ...
    if field_type == "byte":
        ...
        if nbytes_written != len(value):
            raise Exception(
                f"tried to write {len(value)} bytes, but only wrote {nbytes_written}!?"
            )
        return
    elif field_type == "varint":
        assert count == 1, count
...
class LNSerializer:
        ...
        with open(path, newline="") as f:
            ...
            for row in csvreader:
                ...
                if row[0] == "msgtype":
                    ...
                    msg_type_name = row[1]
                    if for_onion_wire:
                        msg_type_int = _parse_msgtype_intvalue_for_onion_wire(
                            str(row[2])
                        )
                    else:
                    ...
                    if (
                        tlv_stream_name
                        not in self.in_tlv_stream_get_tlv_record_scheme_from_type
                    ):
                        self.in_tlv_stream_get_tlv_record_scheme_from_type[
                            tlv_stream_name
                        ] = OrderedDict()
                    ...
                    assert (
                        tlv_record_name
                        not in self.in_tlv_stream_get_record_type_from_name[
                            tlv_stream_name
                        ]
                    ), f"type collision? for {tlv_stream_name}/{tlv_record_name}"
                    assert (
    ...
    def write_tlv_stream(
        self, *, fd: io.BytesIO, tlv_stream_name: str, **kwargs
    ) -> None:
        ...
        for (
            ...
            tlv_record_name = self.in_tlv_stream_get_record_name_from_type[
                tlv_stream_name
            ][tlv_record_type]
            ...
            with io.BytesIO() as tlv_record_fd:
                ...
                _write_tlv_record(
                    fd=fd, tlv_type=tlv_record_type, tlv_val=tlv_record_fd.getvalue()
                )

    def read_tlv_stream(
        self, *, fd: io.BytesIO, tlv_stream_name: str
        ...
        with io.BytesIO() as fd:
            ...
            for row in scheme:
                if row[0] == "msgtype":
                    ...
                    field_count = _resolve_field_count(
                        field_count_str, vars_dict=kwargs
                    )
                    if field_name == "tlvs":
        ...
        with io.BytesIO(data[2:]) as fd:
            for row in scheme:
                ...
                if row[0] == "msgtype":
                    ...
                    field_count_str = row[4]
                    field_count = _resolve_field_count(
                        field_count_str, vars_dict=parsed
                    )
                    if field_name == "tlvs":
                        tlv_stream_name = field_type
