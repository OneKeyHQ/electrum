...
class Plugin(RevealerPlugin):
    ...
    def settings_widget(self, window):
        return EnterButton(
            _("Printer Calibration"), partial(self.calibration_dialog, window)
        )
    ...
    def setup_dialog(self, window):
        ...
        vbox.addWidget(
            WWLabel(
                "<b>"
                + _("Revealer Secret Backup Plugin")
                + "</b><br>"
        ...
        vbox.addWidget(
            QLabel(
                ...
                + _("Warning")
                + "</b>: "
                + _("Each revealer should be used only once.")
                + "<br>"
                + _(
    ...
    def bcrypt(self, dialog):
        ...
        dialog.show_message(
            "".join(
                [
                    _(
                        "{} encrypted for Revealer {}_{} saved as PNG and PDF at: "
                    ).format(self.was, version, code_id),
                    "<b>",
        ...
        dialog.show_message(
            "".join(
                [
                    ...
                    ": </b>",
                    _(
                        "your seed extension will <b>not</b> be included in the encrypted backup."
                    ),
                ]
            ),
            rich_text=True,
    ...
    def toPdf(self, image):
        ...
        size_h = 2028 + (
            (
                int(self.calibration_h)
                * 2028
                / (2028 - (delta_h * 2) + int(self.calibration_h))
            )
            / 2
        )
        size_v = 1284 + (
            (
                int(self.calibration_v)
                * 1284
                / (1284 - (delta_v * 2) + int(self.calibration_v))
            )
            / 2
        )
    ...
    def calibration_pdf(self, image):
        ...
        painter.setFont(font)
        painter.drawText(
            700, 2177, _("1. Place this paper on a flat and well iluminated surface.")
        )
        painter.drawText(
            700,
            2277,
    ...
    def overlay_marks(self, img, is_cseed=False, calibration_sheet=False):
        ...
        base_img = QImage(
            self.f_size.width(), self.f_size.height(), QImage.Format_ARGB32
        )
        ...
        painter.setPen(pen)
        painter.drawLine(
            0, base_img.height() / 2, total_distance_h, base_img.height() / 2
        )
        painter.drawLine(
            base_img.width() / 2, 0, base_img.width() / 2, total_distance_h
        )
        ...
        if not calibration_sheet:
            if is_cseed:  # its a secret
                ...
                painter.drawLine(
                    (base_img.width() - ((total_distance_h)) - (border_thick / 2) - 2)
                    - qr_size,
                    (base_img.height() - ((total_distance_h)))
                    - ((border_thick * 8))
                    - (border_thick / 2)
            ...
            painter.end()
            cal_img = QImage(
                self.f_size.width() + 100,
                self.f_size.height() + 100,
