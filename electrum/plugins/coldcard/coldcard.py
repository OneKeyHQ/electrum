...
from electrum.util import UserFacingException, bfh, bh2u, versiontuple
from electrum.wallet import Abstract_Wallet, Multisig_Wallet, Standard_Wallet
...
class CKCCClient(HardwareClientBase):
    ...
    def show_address(self, path, addr_fmt):
        ...
        return self.dev.send_recv(
            CCProtocolPacker.show_address(path, addr_fmt), timeout=None
        )

    def show_p2sh_address(self, *args, **kws):
        # prompt user w/ p2sh address, also returns it immediately.
        return self.dev.send_recv(
            CCProtocolPacker.show_p2sh_address(*args, **kws), timeout=None
        )
    ...
    def sign_transaction_start(self, raw_psbt: bytes, *, finalize: bool = False):
        ...
        resp = self.dev.send_recv(
            CCProtocolPacker.sign_transaction(dlen, chk, finalize=finalize),
            timeout=None,
        )
    ...
    def sign_message(self, sequence, message, password):
        ...
        try:
            ...
            self.handler.show_error(
                "Only short (%d max) ASCII messages can be signed."
                % MSG_SIGNING_MAX_LENGTH
            )
            return b""
