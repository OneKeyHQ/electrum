...
class LabelsPlugin(BasePlugin):
    ...
    async def push_thread(self, wallet):
        ...
        for key, value in wallet.labels.items():
            ...
            bundle["labels"].append(
                {"encryptedLabel": encoded_value, "externalId": encoded_key}
            )
    ...
    def pull_xpub(self, xpub):
        return asyncio.run_coroutine_threadsafe(
            self.pull_xpub_thread(xpub), self.get_wallet_loop
        ).result()
    ...
    def push_xpub(self, wallet):
        if not wallet.network:
            raise Exception(_("You are offline."))
        return asyncio.run_coroutine_threadsafe(
            self.push_xpub_thread(wallet), wallet.network.asyncio_loop
        ).result()
    ...
    def create_new_wallet(self, wallet, wallet_type, wallet_name):
        ...
        asyncio.run_coroutine_threadsafe(
            self.push_xpub_safe_thread(wallet, wallet_type, wallet_name),
    ...
    def pull_tx(self, wallet):
        if not wallet.network:
            raise Exception(_("You are offline."))
        return asyncio.run_coroutine_threadsafe(
            self.pull_tx_thread(wallet), wallet.network.asyncio_loop
        ).result()
    ...
    def push_tx(self, wallet, action, tx_hash, tx=None, tx_hash_old=None):
        if not wallet.network:
            raise Exception(_("You are offline."))
        return asyncio.run_coroutine_threadsafe(
            self.push_tx_thread(wallet, action, tx_hash, tx, tx_hash_old),
            wallet.network.asyncio_loop,
        ).result()
    ...
    async def push_tx_thread(self, wallet, action, tx_hash, tx=None, tx_hash_old=None):
        ...
        if not wallet_data:
            raise Exception("Wallet {} not loaded".format(wallet))
        wallet_id = wallet_data[2]
        bundle = {"walletId": wallet_id, "txHash": self.encode(wallet, tx_hash)}
        if action != "deltx":
    ...
    async def pull_tx_thread(self, wallet):
        ...
        for tx in response["Transactions"]:
            ...
            try:
                ...
                result["tx_hash"] = self.decode(
                    wallet, tx["TxHash"]
                )  # self.decode_xpub(xpub, wallet['xpubId'])
