...
class DigitalBitbox_Client(HardwareClientBase):
    ...
    def stretch_key(self, key: bytes):
        return to_hexstr(
            hashlib.pbkdf2_hmac("sha512", key, b"Digital Bitbox", iterations=20480)
        )
    ...
    def mobile_pairing_dialog(self):
        ...
        if reply == 0:
            ...
            self.plugin.digitalbitbox_config[CHANNEL_ID_KEY] = dbb_config[
                CHANNEL_ID_KEY
            ]
...
class DigitalBitbox_KeyStore(Hardware_KeyStore):
    ...
    def sign_message(self, sequence, message, password):
        ...
        try:
            ...
            msg = ('{"sign":{"meta":"sign message", "data":%s}}' % hasharray).encode(
                "utf8"
            )
    ...
    def sign_transaction(self, tx, password):
        ...
        try:
            ...
            for step in range(int(steps)):
                ...
                msg = {
                    "sign": {"data": hashes, "checkpub": pubkeyarray,},
                }
            ...
            for i, txin in enumerate(tx.inputs()):
                for pubkey_bytes in txin.pubkeys:
                    ...
                    tx.add_signature_to_txin(
                        txin_idx=i, signing_pubkey=pubkey_bytes.hex(), sig=sig
                    )
...
class DigitalBitboxPlugin(HW_PluginBase):
    ...
    def show_address(self, wallet, address, keystore=None):
        ...
        if wallet.get_txin_type(address) != "p2pkh":
            keystore.handler.show_error(
                _(
                    "This function is only available for p2pkh keystores when using {}."
                ).format(self.device)
            )
            return
