...
from electrum.util import UserCancelled, UserFacingException, bfh, bh2u
...
class KeepKeyPlugin(HW_PluginBase):
    ...
    def __init__(self, parent, config, name):
        ...
        try:
            ...
            self.device_manager().register_devices(
                keepkeylib.transport_hid.DEVICE_IDS, plugin=self
            )
    ...
    def _initialize_device(self, settings, method, device_id, wizard, handler):
        ...
        if method == TIM_NEW:
            ...
            client.load_device_by_mnemonic(
                str(item), pin, passphrase_protection, label, language
            )
        else:
            pin = pin_protection  # It's the pin, not a boolean
    ...
    def tx_inputs(
        self, tx: Transaction, *, for_sig=False, keystore: "KeepKey_KeyStore" = None
    ):
        inputs = []
        for txin in tx.inputs():
