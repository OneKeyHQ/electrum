...
class HTLCManager:
    ...
    def send_htlc(self, htlc: UpdateAddHtlc) -> UpdateAddHtlc:
        ...
        self.log[LOCAL]["locked_in"][htlc_id] = {
            LOCAL: None,
            REMOTE: self.ctn_latest(REMOTE) + 1,
        }
    ...
    def recv_htlc(self, htlc: UpdateAddHtlc) -> None:
        ...
        self.log[REMOTE]["locked_in"][htlc_id] = {
            LOCAL: self.ctn_latest(LOCAL) + 1,
            REMOTE: None,
        }
    ...
    def send_update_fee(self, feerate: int) -> None:
        fee_update = FeeUpdate(
            rate=feerate, ctn_local=None, ctn_remote=self.ctn_latest(REMOTE) + 1
        )
        self._new_feeupdate(fee_update, subject=LOCAL)

    def recv_update_fee(self, feerate: int) -> None:
        fee_update = FeeUpdate(
            rate=feerate, ctn_local=self.ctn_latest(LOCAL) + 1, ctn_remote=None
        )
    ...
    def _new_feeupdate(self, fee_update: FeeUpdate, subject: HTLCOwner) -> None:
        ...
        if (
            last_fee_update.ctn_local is None
            or last_fee_update.ctn_local > self.ctn_latest(LOCAL)
        ) and (
    ...
    def send_rev(self) -> None:
        ...
        for k, fee_update in list(self.log[REMOTE]["fee_updates"].items()):
            if (
                fee_update.ctn_remote is None
                and fee_update.ctn_local <= self.ctn_latest(LOCAL)
            ):
    ...
    def recv_rev(self) -> None:
        ...
        for k, fee_update in list(self.log[LOCAL]["fee_updates"].items()):
            if (
                fee_update.ctn_local is None
                and fee_update.ctn_remote <= self.ctn_latest(REMOTE)
            ):
            ...
            for log_action in ("settles", "fails"):
                for htlc_id in list(self._maybe_active_htlc_ids[htlc_proposer]):
                    ...
                    if (
                        ...
                        and ctns[LOCAL]
                        <= self.ctn_oldest_unrevoked(LOCAL) - sanity_margin
                        and ctns[REMOTE] is not None
                        and ctns[REMOTE]
                        <= self.ctn_oldest_unrevoked(REMOTE) - sanity_margin
                    ):
                        self._maybe_active_htlc_ids[htlc_proposer].remove(htlc_id)
                        if log_action == "settles":
                            htlc = self.log[htlc_proposer]["adds"][
                                htlc_id
                            ]  # type: UpdateAddHtlc
    ...
    def get_unacked_local_updates(self) -> Dict[int, Sequence[bytes]]:
        ...
        return {
            int(ctn): [bfh(msg) for msg in messages]
    ...
    def htlcs(
        ...
        l += [
            (RECEIVED, x)
            for x in self.htlcs_by_direction(subject, RECEIVED, ctn).values()
        ]
        return l
    ...
    def was_htlc_preimage_released(
        self, *, htlc_id: int, htlc_sender: HTLCOwner
    ) -> bool:
    ...
    def all_settled_htlcs_ever(
        ...
        sent = [
            (SENT, x)
            for x in self.all_settled_htlcs_ever_by_direction(subject, SENT, ctn)
        ]
        received = [
            (RECEIVED, x)
            for x in self.all_settled_htlcs_ever_by_direction(subject, RECEIVED, ctn)
        ]
    ...
    def get_feerate(self, subject: HTLCOwner, ctn: int) -> int:
        ...
        assert not (
            ...
            and len(self.log[REMOTE]["fee_updates"]) > 1
        )
        ...
        while True:
            i = (left + right) // 2
            ctn_at_i = (
                fee_log[i].ctn_local if subject == LOCAL else fee_log[i].ctn_remote
            )
            if right - left <= 1:
                break
