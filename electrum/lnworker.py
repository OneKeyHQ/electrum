...
from . import constants, keystore, lnsweep, lnutil, util
from .address_synchronizer import TX_HEIGHT_LOCAL
from .bip32 import BIP32Node
from .bitcoin import COIN, redeem_script_to_address
...
from .lnchannel import Channel, ChannelBackup, ChannelState, PeerState
from .lnmsg import decode_msg
from .lnonion import OnionFailureCode, OnionPacket, process_onion_packet
...
from .lntransport import LNResponderTransport, LNTransport
from .lnutil import (
    LN_MAX_FUNDING_SAT,
    LOCAL,
...
class LNWorker(Logger, NetworkRetryManager[LNPeerAddr]):
    def __init__(self, xprv):
        ...
        self.node_keypair = generate_keypair(
            BIP32Node.from_xkey(xprv), LnKeyFamily.NODE_KEY
        )
    ...
    async def maybe_listen(self):
        ...
        if listen_addr:
            ...
            async def cb(reader, writer):
                transport = LNResponderTransport(
                    self.node_keypair.privkey, reader, writer
                )
                try:
    ...
    async def _add_peer(self, host: str, port: int, node_id: bytes) -> Peer:
        ...
        transport = LNTransport(
            self.node_keypair.privkey, peer_addr, proxy=self.network.proxy
        )
    ...
    def stop(self):
        asyncio.run_coroutine_threadsafe(
            self.taskgroup.cancel_remaining(), self.network.asyncio_loop
        )
    ...
    async def _get_next_peers_to_try(self) -> Sequence[LNPeerAddr]:
        ...
        unconnected_nodes = self.channel_db.get_200_randomly_sorted_nodes_not_in(
            self.peers.keys()
        )
    ...
    def _get_peers_from_dns_seeds(self) -> Sequence[LNPeerAddr]:
        ...
        try:
            ...
            srv_answers = resolve_dns_srv(
                "r{}.{}".format(constants.net.LN_REALM_BYTE, dns_seed)
            )
        except dns.exception.DNSException as e:
            self.logger.info(
                f'failed querying (1) dns seed "{dns_seed}" for ln peers: {repr(e)}'
            )
            return []
    ...
    @staticmethod
    def choose_preferred_address(
        addr_list: Sequence[Tuple[str, int, int]]
    ) -> Tuple[str, int, int]:
    ...
    async def add_peer(self, connect_str: str) -> Peer:
        ...
        if not peer:
            if rest is not None:
                ...
                if not addrs:
                    raise ConnStringFormatError(
                        _("Don't know any addresses for node:") + " " + bh2u(node_id)
                    )
                host, port, timestamp = self.choose_preferred_address(list(addrs))
            port = int(port)
            ...
            try:
                ...
                raise ConnStringFormatError(
                    _("Hostname does not resolve (getaddrinfo failed)")
                )
            # add peer
...
class LNGossip(LNWorker):
    ...
    def start_network(self, network: "Network"):
        ...
        asyncio.run_coroutine_threadsafe(
            self.taskgroup.spawn(self.maintain_db()), self.network.asyncio_loop
        )
...
class LNWallet(LNWorker):
    ...
    def __init__(self, wallet: "Abstract_Wallet", xprv):
        ...
        self.payments = self.db.get_dict(
            "lightning_payments"
        )  # RHASH -> amount, direction, is_paid  # FIXME amt should be msat
        self.preimages = self.db.get_dict("lightning_preimages")  # RHASH -> preimage
        self.sweep_address = (
            wallet.get_new_sweep_address_for_channel()
        )  # TODO possible address-reuse
        self.logs = defaultdict(
            list
        )  # type: Dict[str, List[PaymentAttemptLog]]  # key is RHASH  # (not persisted)
        self.is_routing = (
            set()
        )  # (not persisted) keys of invoices that are in PR_ROUTING state
        # used in tests
        ...
        for channel_id, c in random_shuffled_copy(channels.items()):
            self._channels[bfh(channel_id)] = Channel(
                c, sweep_address=self.sweep_address, lnworker=self
            )

        self.pending_payments = defaultdict(
            asyncio.Future
        )  # type: Dict[bytes, asyncio.Future[BarePaymentAttemptLog]]
    ...
    async def sync_with_remote_watchtower(self):
        while True:
            ...
            if not (
                ...
                or is_private_netaddress(parsed_url.hostname)
            ):
                self.logger.warning(
            ...
            try:
                ...
                self.logger.info(
                    f"could not contact remote watchtower {watchtower_url}"
                )
    ...
    def start_network(self, network: "Network"):
        ...
        for coro in [
            self.maybe_listen(),
            self.lnwatcher.on_network_update(
                "network_updated"
            ),  # shortcut (don't block) if funding tx locked and verified
    ...
    def get_lightning_history(self):
        ...
        for key, plist in self.get_settled_payments().items():
            ...
            if info is not None:
                ...
                direction = (
                    ("sent" if info.direction == SENT else "received")
                    if len(plist) == 1
                    else "self-payment"
                )
            else:
            ...
            if swap:
                if swap.is_reverse:
                    ...
                    item["group_label"] = (
                        "Forward swap"
                        + " "
                        + self.config.format_amount_and_units(swap.onchain_amount)
                    )
            # done
    ...
    def get_onchain_history(self):
        ...
        for chan in self.channels.values():
            ...
            item = {
                ...
                "label": self.wallet.get_label(funding_txid)
                or (_("Open channel") + " " + chan.get_id_for_log()),
                "txid": funding_txid,
            ...
            item = {
                "channel_id": bh2u(chan.channel_id),
                "txid": closing_txid,
                "label": self.wallet.get_label(closing_txid)
                or (_("Close channel") + " " + chan.get_id_for_log()),
    ...
    async def on_channel_update(self, chan):
        ...
        if chan.get_state() == ChannelState.OPEN and chan.should_be_closed_due_to_expiring_htlcs(
    ...
    def open_channel(
        ...
        *,
        ...
        if funding_sat > LN_MAX_FUNDING_SAT:
            raise Exception(
                _("Requested channel capacity is over protocol allowed maximum.")
            )
    ...
    def pay(
        self, invoice: str, *, amount_msat: int = None, attempts: int = 1
        ...
        for i in range(attempts):
            try:
                ...
                route = await run_in_thread(
                    partial(
                        self._create_route_from_invoice, lnaddr, full_path=full_path
                    )
                )
    ...
    async def _pay_to_route(
        self, route: LNPaymentRoute, lnaddr: LnAddr
    ) -> PaymentAttemptLog:
        ...
        if payment_attempt.success:
            ...
            if payment_attempt.error_bytes:
                ...
                failure_msg, sender_idx = chan.decode_onion_error(
                    payment_attempt.error_bytes, route, htlc.htlc_id
                )
                is_blacklisted = self.handle_error_code_from_failed_htlc(
                    failure_msg, sender_idx, route, peer
                )
                if is_blacklisted:
    ...
    def handle_error_code_from_failed_htlc(self, failure_msg, sender_idx, route, peer):
        ...
        if code in failure_codes:
            ...
            channel_update_len = int.from_bytes(
                data[offset : offset + 2], byteorder="big"
            )
            channel_update_as_received = data[
                offset + 2 : offset + 2 + channel_update_len
            ]
            payload = self._decode_channel_update_msg(channel_update_as_received)
            if payload is None:
                self.logger.info(
                    f"could not decode channel_update for failed htlc: {channel_update_as_received.hex()}"
                )
                return True
            ...
            if r == UpdateStatus.GOOD:
                ...
                self.channel_db.add_channel_update_for_private_channel(
    ...
    @classmethod
    def _decode_channel_update_msg(
        cls, chan_upd_msg: bytes
    ...
    def _check_invoice(invoice: str, *, amount_msat: int = None) -> LnAddr:
        ...
        if (
            amount_msat
        ):  # replace amt in invoice. main usecase is paying zero amt invoices
            existing_amt_msat = addr.get_amount_msat()
            if existing_amt_msat and amount_msat < existing_amt_msat:
                raise Exception(
                    "cannot pay lower amt than what is originally in LN invoice"
                )
            addr.amount = Decimal(amount_msat) / COIN / 1000
        if addr.amount is None:
            raise InvoiceError(_("Missing amount"))
        if (
            addr.get_min_final_cltv_expiry()
            > lnutil.NBLOCK_CLTV_EXPIRY_TOO_FAR_INTO_FUTURE
        ):
            raise InvoiceError(
                "{}\n{}".format(
    ...
    def _create_route_from_invoice(
        self, decoded_invoice: "LnAddr", *, full_path: LNPaymentPath = None
    ) -> LNPaymentRoute:
        ...
        for private_route in r_tags:
            ...
            if full_path:
                ...
                if [
                    edge.short_channel_id for edge in full_path[-len(private_route) :]
                ] != [edge[1] for edge in private_route]:
                    continue
            ...
            try:
                route = self.network.path_finder.create_route_from_path(
                    path, self.node_keypair.pubkey, my_channels=scid_to_my_channels
                )
            except NoChannelPolicy:
                continue
            # we need to shift the node pubkey by one towards the destination:
            private_route_nodes = [edge[0] for edge in private_route][1:] + [
                invoice_pubkey
            ]
            ...
            for node_pubkey, edge_rest in zip(private_route_nodes, private_route_rest):
                ...
                node_info = self.channel_db.get_node_info_for_node_id(
                    node_id=node_pubkey
                )
                route.append(
                    RouteEdge(
            ...
            if not is_route_sane_to_use(
                route, amount_msat, decoded_invoice.get_min_final_cltv_expiry()
            ):
    ...
    async def create_invoice(self, amount_sat: Optional[int], message, expiry: int):
        ...
        lnaddr = LnAddr(
            ...
            + routing_hints,
            date=timestamp,
            payment_secret=derive_payment_secret_from_payment_preimage(
                payment_preimage
            ),
        )
    ...
    async def _add_request_coro(
        self, amount_sat: Optional[int], message, expiry: int
    ) -> str:
    ...
    def payment_failed(
        self, chan, payment_hash: bytes, error_bytes: bytes, failure_message
    ):
        ...
        if f and not f.cancelled():
            payment_attempt = BarePaymentAttemptLog(
                success=False, error_bytes=error_bytes, failure_message=failure_message
            )
            f.set_result(payment_attempt)
        else:
            chan.logger.info(
                "received unexpected payment_failed, probably from previous session"
            )
            key = payment_hash.hex()
            util.trigger_callback("invoice_status", key)
    ...
    def payment_sent(self, chan, payment_hash: bytes):
        ...
        f = self.pending_payments.get(payment_hash)
        if f and not f.cancelled():
            payment_attempt = BarePaymentAttemptLog(success=True, preimage=preimage)
            f.set_result(payment_attempt)
        else:
            chan.logger.info(
                "received unexpected payment_sent, probably from previous session"
            )
            key = payment_hash.hex()
            util.trigger_callback("invoice_status", key)
            util.trigger_callback("payment_succeeded", self.wallet, key)
    ...
    async def _calc_routing_hints_for_invoice(self, amount_sat: Optional[int]):
        ...
        random.shuffle(
            channels
        )  # not sure this has any benefit but let's not leak channel order
        scid_to_my_channels = {
            chan.short_channel_id: chan
            for chan in channels
            if chan.short_channel_id is not None
        ...
        for chan in channels:
            ...
            channel_info = self.channel_db.get_channel_info(
                chan_id, my_channels=scid_to_my_channels
            )
            ...
            missing_info = True
            if channel_info:
                policy = self.channel_db.get_policy_for_node(
    ...
    def num_sats_can_send(self) -> Union[Decimal, int]:
        with self.lock:
            return (
                Decimal(
                    max(
                        chan.available_to_spend(LOCAL) if chan.is_open() else 0
                        for chan in self.channels.values()
                    )
                )
                / 1000
    ...
    def num_sats_can_receive(self) -> Union[Decimal, int]:
        with self.lock:
            return (
                Decimal(
                    max(
                        chan.available_to_spend(REMOTE) if chan.is_open() else 0
                        for chan in self.channels.values()
                    )
                )
                / 1000
    ...
    def export_channel_backup(self, channel_id):
        ...
        assert (
            backup_bytes == ChannelBackupStorage.from_bytes(backup_bytes).to_bytes()
        ), "roundtrip failed"
        encrypted = pw_encode_with_version_and_mac(backup_bytes, xpub)
        assert backup_bytes == pw_decode_with_version_and_mac(
            encrypted, xpub
        ), "encrypt failed"
...
class LNBackups(Logger):
    ...
    def __init__(self, wallet: "Abstract_Wallet"):
        ...
        for channel_id, cb in random_shuffled_copy(
            self.db.get_dict("channel_backups").items()
        ):
            self.channel_backups[bfh(channel_id)] = ChannelBackup(
                cb, sweep_address=self.sweep_address, lnworker=self
            )
    ...
    def start_network(self, network: "Network"):
        ...
        for cb in self.channel_backups.values():
            self.lnwatcher.add_channel(
                cb.funding_outpoint.to_str(), cb.get_funding_address()
            )
    ...
    def import_channel_backup(self, data):
        ...
        self.channel_backups[bfh(channel_id)] = cb = ChannelBackup(
            cb_storage, sweep_address=self.sweep_address, lnworker=self
        )
        self.wallet.save_db()
        util.trigger_callback("channels_updated", self.wallet)
        self.lnwatcher.add_channel(
