...
from electrum.invoices import PR_FAILED, PR_PAID
...
from electrum.wallet import Abstract_Wallet, InternalAddressCorruption, Wallet
from electrum.wallet_db import WalletDB

from .i18n import _
from .uix.dialogs import (
    InfoBubble,
    OutputItem,
    OutputList,
    RefLabel,
...
from .uix.dialogs.password_dialog import (
...
class ElectrumWindow(App):
    ...
    def choose_server_dialog(self, popup):
        ...
        ChoiceDialog(
            _("Choose a server"), server_choices, popup.ids.server_str.text, cb2
        ).open()
    ...
    def btc_to_fiat(self, amount_str):
        ...
        if rate.is_nan():
            return ""
        fiat_amount = (
            self.get_amount(amount_str + " " + self.base_unit) * rate / pow(10, 8)
        )
    ...
    def __init__(self, **kwargs):
        ...
        self._trigger_update_history = Clock.create_trigger(self.update_history, 0.5)
        self._trigger_update_interfaces = Clock.create_trigger(
            self.update_interfaces, 0.5
        )

        self._periodic_update_status_during_sync = Clock.schedule_interval(
            self.update_wallet_synchronizing_progress, 0.5
        )
        self._periodic_process_queued_invoice = Clock.schedule_interval(
            self._maybe_process_queued_invoice, 0.5
        )
    ...
    def qr_dialog(
        self, title, data, show_text=False, text_for_clipboard=None, help_text=None
    ):
    ...
    def load_wallet_by_name(self, path, ask_if_wizard=False):
        ...
        if self.wallet and self.wallet.storage.path == path:
            ...
            if not ask_if_wizard:
                ...
                d = Question(
                    _("Do you want to launch the wizard again?"), handle_answer
                )
                d.open()
    ...
    def popup_dialog(self, name):
        if name == "settings":
            ...
            popup = Builder.load_file(
                "electrum/gui/kivy/uix/ui_screens/" + name + ".kv"
            )
    ...
    def init_ui(self):
        ...
        Factory.register("AnimatedPopup", module="electrum.gui.kivy.uix.dialogs")
        Factory.register("QRCodeWidget", module="electrum.gui.kivy.uix.qrcodewidget")
    ...
    def get_max_amount(self):
        ...
        return format_satoshis_plain(
            amount_after_all_fees, decimal_point=self.decimal_point()
        )
    ...
    def format_amount_and_units(self, x) -> str:
        ...
        if x == "!":
            return "max"
        return (
            format_satoshis_plain(x, decimal_point=self.decimal_point())
            + " "
            + self.base_unit
        )
    ...
    def show_info_bubble(
        ...
        if info_bubble.parent:
            win.remove_widget(
                info_bubble if not info_bubble.modal else info_bubble._modal_view
            )
        ...
        if text == "texture":
            ...
            info_bubble.background_image = (
                "atlas://electrum/gui/kivy/theming/light/card"
            )
        else:
    ...
    def broadcast(self, tx):
        ...
        if self.network and self.network.is_connected():
            ...
            threading.Thread(
                target=self._broadcast_thread, args=(tx, on_complete)
            ).start()
        else:
            self.show_info(
    ...
    def _delete_wallet(self, b):
        if b:
            ...
            self.protected(
                _("Are you sure you want to delete wallet {}?").format(basename),
    ...
    def export_private_keys(self, pk_label, addr):
        ...
        self.protected(
            _("Decrypt your private key?"), show_private_key, (addr, pk_label)
        )

    def import_channel_backup(self, encrypted):
        d = Question(
            _("Import Channel Backup?"),
            lambda b: self._import_channel_backup(b, encrypted),
        )
        d.open()
