...
from electrum import bitcoin, constants, simple_config
...
from electrum.simple_config import FEE_RATIO_HIGH_WARNING, FEERATE_WARNING_HIGH_FEE
...
class CScreen(Factory.Screen):
    ...
    def on_enter(self):
        ...
        Clock.schedule_once(lambda dt: self.dispatch("on_activate"), 0.25)
        pass
...
class HistoryScreen(CScreen):
    ...
    def get_card(self, tx_item):  # tx_hash, tx_mined_status, value, balance):
        ...
        if is_lightning:
            status = 0
            status_str = (
                "unconfirmed" if timestamp is None else format_time(int(timestamp))
            )
...
class SendScreen(CScreen):
    ...
    def set_ln_invoice(self, invoice: str):
        try:
            ...
            self.app.show_info(
                invoice + _(" is not a valid Lightning invoice: ") + repr(e)
            )  # repr because str(Exception()) == ''
            return
        self.address = invoice
        self.message = dict(lnaddr.tags).get("d", None)
        self.amount = (
            self.app.format_amount_and_units(lnaddr.amount * bitcoin.COIN)
            if lnaddr.amount
            else ""
        )
        self.payment_request = None
    ...
    def _do_pay_onchain(self, invoice: OnchainInvoice, rbf: bool) -> None:
        ...
        fee = tx.get_fee()
        amount = (
            sum(map(lambda x: x.value, outputs))
            if "!" not in [x.value for x in outputs]
...
class ReceiveScreen(CScreen):
    ...
    def expiry(self):
        return self.app.electrum_config.get(
    ...
    def new_request(self, lightning):
        ...
        if lightning:
            ...
            if not addr:
                self.app.show_info(
                    _(
                        "No address available. Please remove some of your pending requests."
                    )
                )
                return
            self.address = addr
            req = self.app.wallet.make_payment_request(
                addr, amount, message, self.expiry()
            )
    ...
    def expiration_dialog(self, obj):
        ...
        d = ChoiceDialog(
            _("Expiration date"), pr_expiration_values, self.expiry(), callback
        )
        d.open()
...
class TabbedCarousel(Factory.TabbedPanel):
    ...
    def animate_tab_to_center(self, value):
        ...
        if idx in [0, 1]:
            ...
            scroll_x = 1.0 * (n - idx - 1) / (n - 1)
        mation = Factory.Animation(scroll_x=scroll_x, d=0.25)
