...
from electrum.lnchannel import AbstractChannel, Channel, htlcsum
...
from .util import Buttons, ButtonsLineEdit, CloseButton
...
class ChannelDetailsDialog(QtWidgets.QDialog):
    ...
    def update(self):
        self.can_send_label.setText(
            self.format_msat(self.chan.available_to_spend(LOCAL))
        )
        self.can_receive_label.setText(
            self.format_msat(self.chan.available_to_spend(REMOTE))
        )
        self.sent_label.setText(self.format_msat(self.chan.total_msat(Direction.SENT)))
        self.received_label.setText(
            self.format_msat(self.chan.total_msat(Direction.RECEIVED))
        )
    ...
    def __init__(self, window: "ElectrumWindow", chan_id: bytes):
        ...
        self.dust_limit = SelectableLabel(
            self.window.format_amount_and_units(chan.config[REMOTE].dust_limit_sat)
        )
