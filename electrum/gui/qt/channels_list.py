...
from PyQt5.QtGui import QBrush, QFont, QStandardItem
...
from electrum.lnutil import LN_MAX_FUNDING_SAT, LOCAL, REMOTE, format_short_channel_id
from electrum.lnworker import LNWallet
from electrum.util import NoDynamicFeeEstimates, NotEnoughFunds, bh2u
...
class ChannelsList(MyTreeView):
    ...
    def format_fields(self, chan):
        ...
        for subject in (REMOTE, LOCAL):
            ...
            if bal_other != bal_minus_htlcs_other:
                label += (
                    " (+"
                    + self.parent.format_amount(bal_other - bal_minus_htlcs_other)
                    + ")"
                )
    ...
    def force_close(self, channel_id):
        ...
        msg = (
            ...
            + "\n\n"
            + _(
                "Funds retrieved from this channel will not be available before {} blocks after forced closure."
            ).format(to_self_delay)
            + " "
            + _(
                "After that delay, funds will be sent to an address derived from your wallet seed."
            )
            + "\n\n"
            + _(
                "In the meantime, channel funds will not be recoverable from your seed, and might be lost if you lose your wallet."
            )
            + " "
            + _(
                "To prevent that, you should have a backup of this channel on another device."
            )
        )
    ...
    def export_channel_backup(self, channel_id):
        ...
        self.main_window.show_qrcode(
            data, "channel backup", help_text=msg, show_copy_text_btn=True
        )
    ...
    def create_menu(self, position):
        ...
        if not item:
            return
        channel_id = idx.sibling(idx.row(), self.Columns.NODE_ALIAS).data(
            ROLE_CHANNEL_ID
        )
        if channel_id in self.lnbackups.channel_backups:
            menu.addAction(
                _("Request force-close"), lambda: self.request_force_close(channel_id)
            )
        ...
        if not chan.is_closed():
            if not chan.is_frozen_for_sending():
                menu.addAction(
                    _("Freeze (for sending)"), lambda: chan.set_frozen_for_sending(True)
                )
            else:
                menu.addAction(
        ...
        if not chan.is_closed():
            ...
            if chan.peer_state == PeerState.GOOD:
                menu.addAction(
                    _("Close channel"), lambda: self.close_channel(channel_id)
                )
            menu.addAction(
                _("Force-close channel"), lambda: self.force_close(channel_id)
            )
        else:
        ...
        menu.addSeparator()
        menu.addAction(
            _("Export backup"), lambda: self.export_channel_backup(channel_id)
        )
        if chan.is_redeemed():
            menu.addSeparator()
    ...
    def _update_chan_frozen_bg(
        self, *, chan: AbstractChannel, items: Sequence[QStandardItem]
    ):
        ...
        if chan.is_frozen_for_sending():
            ...
            item.setToolTip(
                _(
                    "This channel is frozen for sending. It will not be used for outgoing payments."
                )
            )
        else:
        ...
        if chan.is_frozen_for_receiving():
            ...
            item.setToolTip(
                _(
                    "This channel is frozen for receiving. It will not be included in invoices."
                )
            )
        else:
            item.setBackground(self._default_item_bg_brush)
    ...
    def update_can_send(self, lnworker: LNWallet):
        msg = (
            _("Can send")
            + " "
            + self.parent.format_amount(lnworker.num_sats_can_send())
            + " "
            + self.parent.base_unit()
            + "; "
            + _("can receive")
            + " "
            + self.parent.format_amount(lnworker.num_sats_can_receive())
            + " "
            + self.parent.base_unit()
        )
    ...
    def get_toolbar(self):
        ...
        self.new_channel_button = EnterButton(
            _("Open Channel"), self.new_channel_dialog
        )
    ...
    def statistics_dialog(self):
        ...
        capacity = (
            self.parent.format_amount(channel_db.capacity())
            + " "
            + self.parent.base_unit()
        )
