...
class SwapDialog(WindowModalDialog):
    ...
    def _spend_max_reverse_swap(self):
        amount = min(
            self.lnworker.num_sats_can_send(), self.swap_manager.get_max_amount()
        )
        self.send_amount_e.setAmount(amount)
    ...
    def on_send_edited(self):
        ...
        if (
            self.is_reverse
            and send_amount
            and send_amount > self.lnworker.num_sats_can_send()
        ):
            recv_amount = None
    ...
    def update(self):
        ...
        self.send_button.setIcon(
            read_QIcon("lightning.png" if self.is_reverse else "bitcoin.png")
        )
        self.send_button.repaint()  # macOS hack for #6269
        self.recv_button.setIcon(
            read_QIcon("lightning.png" if not self.is_reverse else "bitcoin.png")
        )
    ...
    def update_fee(self):
        ...
        fee_text = (
            self.window.format_amount(fee) + " " + self.window.base_unit()
            if fee
            else ""
        )
    ...
    def update_tx(self, onchain_amount):
        if onchain_amount is None:
            ...
            return
        outputs = [
            PartialTxOutput.from_address_and_value(ln_dummy_address(), onchain_amount)
        ]
    ...
    def do_normal_swap(self, lightning_amount, onchain_amount, password):
        coro = self.swap_manager.normal_swap(
            lightning_amount, onchain_amount, password, tx=self.tx
        )
        self.window.run_coroutine_from_thread(coro)
