...
class ElectrumGui:
    ...
    def print_contacts(self):
        messages = map(
            lambda x: "%20s   %45s " % (x[0], x[1][1]), self.contacts.items()
        )
    ...
    def print_edit_line(self, y, label, text, index, size):
        ...
        self.stdscr.addstr(
            ...
            15,
            text,
            curses.A_REVERSE if self.pos % 6 == index else curses.color_pair(1),
        )
    ...
    def run_contacts_tab(self, c):
        if c == 10 and self.contacts:
            out = self.run_popup(
                "Address", ["Copy", "Pay to", "Edit label", "Delete"]
            ).get("button")
    ...
    def do_send(self):
        ...
        try:
            tx = self.wallet.mktx(
                outputs=[
                    PartialTxOutput.from_address_and_value(self.str_recipient, amount)
                ],
                password=password,
                fee=fee,
            )
        ...
        if out:
            ...
            if out.get("server") or out.get("proxy"):
                ...
                net_params = NetworkParameters(
                    server=server_addr, proxy=proxy, auto_connect=auto_connect
                )
    ...
    def run_dialog(self, title, items, interval=2, buttons=None, y_pos=3):
        ...
        self.w = curses.newwin(
            5 + len(list(items)) * interval + (2 if buttons else 0), 50, y_pos, 5
        )
        w = self.w
        out = {}
        while True:
